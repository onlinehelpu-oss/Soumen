...
- Broker Abstraction & Auth:
    ...
    * Supports static IP configuration binding / reporting for API compliance.
...
class BaseBrokerClient(ABC):
    ...
    @abstractmethod
    def place_order(self, symbol: str, qty: int, side: int, product_type: str) -> dict:
        pass

    @abstractmethod
    def place_gtt(self, symbol: str, qty: int, trigger_price: float, product_type: str) -> dict:
        pass
...
class FyersBrokerAdapter(BaseBrokerClient):
    """Fyers v3 Broker Implementation with Static IP reporting and session support."""
    ...
    def cancel_gtt(self, gtt_id: str) -> dict:
        if self.model is None:
            return {"s": "error", "message": "Fyers model not initialized"}
        return self.model.cancel_gtt(id=gtt_id)
        ...
        try:
            ...
            if isinstance(data, dict):
                ...
                if api_key and api_secret and redirect_url:
                    return {"api_key": api_key, "api_secret": api_secret, "redirect_url": redirect_url, "primary_ip": PRIMARY_STATIC_IP}
        except Exception:
            pass

    _real_print("---- Enter your Broker Login Credentials ----")
    creds = {
        "api_key": input("Enter APP ID: ").strip(),
...
def validate_authcode(app_id, secret_id, auth_code, max_retries=5):
    ...
    if PRIMARY_STATIC_IP:
        _real_print(f"[auth] Binding API validation requests to Static IP: {PRIMARY_STATIC_IP}")
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            data = r.json()
            if data.get("s") == "error":
                raise RuntimeError(f"Broker error {data.get('code')}: {data.get('message')}")
            return data
...
def place_gtt_stoploss(symbol: str, qty: int, trigger_price: float) -> dict:
    if BROKER_CLIENT is None:
        err = {"s": "error", "message": "no broker client for gtt"}
        log_trade_event(symbol, "GTT_FAIL", qty, trigger_price, err)
        return err
    for attempt in range(1, 3):
        try:
            resp = BROKER_CLIENT.place_gtt(symbol=symbol, qty=qty, trigger_price=trigger_price, product_type=PRODUCT_TYPE)
...
def main():
    ...
    if args.test_table or fyersModel is None:
        ...
        _real_print(f"PRIMARY_IP      = {PRIMARY_STATIC_IP}")
