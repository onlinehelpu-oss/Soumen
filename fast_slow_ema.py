    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            if r.status_code == 503:
                ...
                _real_print(
                    f"[{attempt}/{max_retries}] 503 from auth server. Retrying in {sleep_s}s...")
                time.sleep(sleep_s)
                continue
            r.raise_for_status()
            data = r.json()
            if data.get("s") == "error":
                raise RuntimeError(
                    f"Fyers error {data.get('code')}: {data.get('message')}")
            return data
            ...
            sleep_s = min(2 ** attempt, 30)
            _real_print(
                f"[{attempt}/{max_retries}] Network error: {e}. Retrying in {sleep_s}s...")
            time.sleep(sleep_s)
...
def run_interactive_login() -> str:
    ...
    if os.path.exists(TOKEN_PATH):
        try:
            ...
            if access_token:
                ...
                _real_print(
                    f"Access Token (loaded from file) : {access_token}")
                return access_token
        except Exception:
            pass
    ...
    try:
        ...
        _real_print(
            f"[auth] Could not store access_token into {CONFIG_FILE}: {e}")
...
SYMBOL_STATES: Dict[str, SymbolState] = {
    s: SymbolState(s) for s in UNDERLYING_SYMBOLS}
...
if not os.path.exists(LOG_FILE):
    ...
    with open(LOG_FILE, "w", newline="") as f:
        csv.writer(f).writerow(
            ["ts", "symbol", "action", "qty", "price", "response"])
...
def log_trade_event(symbol, action, qty, price, response):
    ...
    with open(LOG_FILE, "a", newline="") as f:
        csv.writer(f).writerow(
            [dt.now().isoformat(), symbol, action, qty, price, resp_text])
...
def resolve_option_symbol(fyers: fyersModel.FyersModel, underlying: str, ltp: float) -> Optional[str]:
    ...
    config = SYMBOL_CONFIG.get(base_symbol_for_api)
    if not config:
        _real_print(f"[debug-options] No config found for {base_symbol_for_api}")
        return None

    # 2. Fetch option chain
    try:
        api_symbol = config['fy_sym']
        payload = {"symbol": api_symbol, "strikecount": 20}
        resp = fyers.optionchain(data=payload)
        if not resp or resp.get("s") != "ok":
            _real_print(f"[debug-options] Failed option chain response for {api_symbol}: {resp}")
            return None
        chain = (resp.get("data") or {}).get("optionsChain") or []
        if not chain:
            _real_print(f"[debug-options] Option chain is empty for {api_symbol}.")
            return None
    except Exception as e:
        _real_print(f"[debug-options] Error fetching option chain for {api_symbol}: {e}")
        return None

    # 3. Find the earliest expiry date
    try:
        expiries = sorted(list(set(
            dt.strptime(row['expiry'], '%Y-%m-%d') for row in chain if 'expiry' in row
        )))
        if not expiries:
            _real_print("[debug-options] No expiry dates found in option chain.")
            return None
...
class CandleManager:
    ...
    def process_tick(self, tick: dict):
        try:
            ...
            if not symbol:
                return
            ltp = tick.get("ltp") or tick.get(
                "last_price") or tick.get("last_traded_price")
            if ltp is None:
                return
            ltp = float(ltp)
...
def place_market_order(symbol: str, qty: int, side: int) -> dict:
    ...
    if side == 1 and len([s for s in SYMBOL_STATES.values() if s.status == "position"]) >= MAX_CONCURRENT_POS:
        _real_print(
            f"[order] MAX_CONCURRENT_POS reached ({MAX_CONCURRENT_POS}). Rejecting new buy for {symbol}.")
...
def get_access_token() -> dict:
    ...
    _real_print(
        "[auth] No existing access token found. Starting interactive login...")
            ...
            if hasattr(fyers_client, "history"):
                try:
                    _ = fyers_client.history(
                        ...
                        range_from=(dt.now() - timedelta(days=2)
                                    ).strftime("%Y-%m-%d"),
...
def evaluate_on_new_candle(st: SymbolState):
    ...
    if st.status == "watch":
        ...
        if (
            ...
            st.potential_target_price = float(
                target) if target is not None and not math.isnan(target) else None

            try:
                sig_start = pd.to_datetime(curr.name)
                if sig_start.tzinfo is not None:
                    sig_start = sig_start.tz_convert(
            ...
            try:
                ...
                if getattr(st, "data", None) is not None and not st.data.empty:
                    ...
                    df = compute_indicators(
                        CANDLE_MANAGER.history.get(sym).copy())
                else:
...
def on_ws_open():
    _real_print(
        f"[ws:open] subscribing to {len(UNDERLYING_SYMBOLS)} symbols...")
    try:
        FYERS_SOCKET.subscribe(symbols=UNDERLYING_SYMBOLS,
        ...
        for sym, st in SYMBOL_STATES.items():
            try:
                ...
                if getattr(st, "status", None) == "position" and getattr(st, "exit_pending",
                    ...
                    _real_print(
                        f"****** [{sym}] EXIT SIGNAL (RED candle crossed & closed below EXIT EMA) ******")
                    _real_print(
...
def _recreate_fyers_and_ws():
    ...
    try:
        ...
        ACCESS_TOKEN = auth.get("access_token") or auth.get(
    ...
    try:
        ...
        _real_print(
            "[ws] Attempting to reconnect websocket with refreshed token...")
...
def _serialize_state():
    ...
    for sym, st in SYMBOL_STATES.items():
        out[sym] = {
            ...
            "opt_symbol": getattr(st, "opt_symbol", None),
            "last_candle_ts": (
...
def load_state_from_disk():
    ...
    try:
        ...
        for sym, info in raw.items():
            ...
            st.opt_symbol = info.get("opt_symbol", st.opt_symbol)
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Options Fast-Slow EMA Strategy (Fyers v3)")
...
def main():
    ...
    ENTRY_FAST_EMA = int(pick("entry_fast_ema", getattr(
        args, "entry_fast_ema", None), ENTRY_FAST_EMA))
    ENTRY_SLOW_EMA = int(pick("entry_slow_ema", getattr(
        args, "entry_slow_ema", None), ENTRY_SLOW_EMA))
    EMA_BUFFER = float(pick("ema_buffer", args.ema_buffer, EMA_BUFFER))
    MIN_RANGE_PCT = float(pick("min_range_pct", getattr(
        args, "min_range_pct", MIN_RANGE_PCT), MIN_RANGE_PCT))
    ...
    try:
        CANDLE_MANAGER = CandleManager(
            TIMEFRAME_MIN, on_candle=on_completed_candle, tz=TIMEZONE)
    except Exception:
        CANDLE_MANAGER = CandleManager(
            TIMEFRAME_MIN, on_candle=on_completed_candle)

    if args.test_table or fyersModel is None:
        FYERS = None
        _real_print(
            "[mode] TEST TABLE mode (no fyers). Warmup synthetic data.")
        for sym in UNDERLYING_SYMBOLS:
            st = SYMBOL_STATES[sym]
            idx = pd.date_range(end=dt.now(), periods=200,
        ...
        client_id = ACCESS_TOKEN.split(
            ":")[0] if ":" in ACCESS_TOKEN else ACCESS_TOKEN
        FYERS = fyersModel.FyersModel(
            client_id=client_id,
            is_async=False,
            token=ACCESS_TOKEN,
        ...
        _real_print(
            "[auth] Fyers model initialized. Running warmup history fetch (best-effort).")
        warmup_all(FYERS)
    ...
    if args.test_table:
        ...
        _real_print(
            "[test] Emitted synthetic candles to evaluator. Exiting (test mode).")
        return
