...
def evaluate_on_new_candle(st: SymbolState):
    ...
    if st.status == "watch":
        ...
        if (
            ...
            _real_print(f"****** [{st.symbol}] ENTRY SIGNAL (BODY crossed from below both EMAs to above both EMAs, EMA_fast>EMA_slow) ******")
                    ...
                    if FYERS is not None and hasattr(FYERS, "history"):
                        try:
                            ...
                            if isinstance(r, dict) and r.get("s") == "ok":
                                df_tmp = pd.DataFrame(r["candles"], columns=["ts", "open", "high", "low", "close", "volume"])
                                df_tmp["ts"] = (pd.to_datetime(df_tmp["ts"], unit="s", utc=True).dt.tz_convert(TIMEZONE).dt.tz_localize(None))
        ...
        for sym, st in SYMBOL_STATES.items():
            try:
                ...
                if getattr(st, "status", None) == "entry_pending" and st.signal_candle:
                    sc = st.signal_candle
                    _real_print(f"****** [{sym}] ENTRY SIGNAL (BODY crossed from below both EMAs to above both EMAs, EMA_fast>EMA_slow) ******")
                    _real_print(f"[signal:{sym}] signal_high={float(sc.get('high')):.2f} signal_low={float(sc.get('low')):.2f} | waiting for NEXT CANDLE to attempt breakout entry")
                if getattr(st, "status", None) == "position" and getattr(st, "exit_pending", False) and st.exit_signal_candle:
                    ec = st.exit_signal_candle
                    _real_print(f"****** [{sym}] EXIT SIGNAL (RED candle crossed & closed below EXIT EMA) ******")
                    _real_print(f"[exit:{sym}] exit_low={float(ec.get('low')):.2f} exit_high={float(ec.get('high')):.2f} | waiting for NEXT CANDLE to attempt exit on break below exit_low")
            except Exception:
                continue
    except Exception:
        pass
    ...
    try:
        ...
        if expired:
            if REAUTH_ATTEMPTS >= MAX_REAUTH_ATTEMPTS:
                _real_print("[ws:error] Token expired, but max re-auth attempts reached. Please restart script and login again.")
                return
            REAUTH_ATTEMPTS += 1
            _real_print(
...
def parse_args():
    ...
    p.add_argument("--min-range-pct", type=float, default=MIN_RANGE_PCT, help="Minimum candle range fraction (e.g., 0.001 for 0.1%%)")
    p.add_argument("--ema-buffer", type=float, default=EMA_BUFFER, help="Buffer above/below EMA to confirm breaks (float)")
    ...
    p.add_argument("--fixed-qty", type=int, default=None, help="When --position-mode qty is set, fixed quantity per trade (default 1).")
    p.add_argument("--qty-map", type=str, default="", help='Optional per-symbol qty map as JSON string, e.g. \'{"NSE:RELIANCE-EQ":2}\'.')
