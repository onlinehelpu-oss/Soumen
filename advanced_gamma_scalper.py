...
MAX_DAILY_LOSS_RUPEES = 5000  # Max 5000 rupees loss per symbol per day
...
class ScalpPosition:
    ...
    index_symbol: str
    side: str  # "LONG", "SHORT"
    entry_price: float  # Actual option premium
    current_price: float  # Current option premium
    quantity: int
    entry_time: float

    index_entry_price: float  # Index price at entry (for reference)
    ...
    def pnl_bp(self) -> float:
        """PnL in basis points (for trailing stop logic)"""
        if self.side == "LONG":
...
class DailyStats:
    ...
    wins: int = 0
    losses: int = 0
    total_pnl_rupees: float = 0.0  # Changed from bp to rupees
    max_drawdown_rupees: float = 0.0  # Changed from bp to rupees
    current_drawdown_rupees: float = 0.0  # Changed from bp to rupees
...
class ScalpPositionManager:
            ...
            try:
                with open(STATE_FILE, 'r') as f:
                    ...
                    for sym, pos_list in data.get('positions', {}).items():
                        restored_positions = []
                        for p in pos_list:
                            if 'index_entry_price' not in p:
                                p['index_entry_price'] = 0.0
                            restored_positions.append(ScalpPosition(**p))
                        self.positions[sym] = restored_positions
                        # Restore stats
    ...
    def can_open_position(self, index_symbol: str) -> Tuple[bool, str]:
        ...
        if stats and stats.total_pnl_rupees < -MAX_DAILY_LOSS_RUPEES:
            return False, f"DAILY_LOSS_LIMIT (₹{stats.total_pnl_rupees:.2f})"
    ...
    def open_position(self, symbol: str, index_symbol: str, side: str,
                      ...
                      index_entry_price: float, metadata: dict = None):
        ...
        # Calculate stops and targets based on option premium
        if side == "LONG":
        ...
        print(
            f"[SCALP] OPEN {side} {symbol} @ ₹{entry_price:.2f} | SL: ₹{stop_loss:.2f} | TGT: ₹{target:.2f} | "
            f"Qty: {quantity} | Rule: {strategy} (SL:{stop_bp}bp TGT:{target_bp}bp)")

    def update_positions(self, index_symbol: str, option_price_map: dict = None):
        """Update all positions for a symbol with current option prices and manage exits

        Args:
            index_symbol: The index symbol (e.g., NSE:FINNIFTY-INDEX)
            option_price_map: Dict mapping option symbols to their current prices
        """
        ...
        for i, pos in enumerate(self.positions[index_symbol]):
            # Update current price from the price map
            if option_price_map and pos.symbol in option_price_map:
                pos.current_price = option_price_map[pos.symbol]
            ...
            if not pos.partial_exit_done and pos.remaining_qty == pos.quantity:
                if pnl_bp >= target_bp_dist * PARTIAL_EXIT_PCT:
                    ...
                    print(
                        f"[SCALP] PARTIAL EXIT {pos.symbol} @ ₹{pos.current_price:.2f} | Closed {partial_qty}, Remaining {pos.remaining_qty} | PnL: ₹{pnl_rupees:+.2f}")
            ...
            if pos.highest_profit_bp >= pos.trailing_activation_bp and not pos.trailing_stop:
                if pos.side == "LONG":
                    ...
                    pos.trailing_stop = pos.current_price * (1 - pos.trailing_step_bp / 10000)
                else:
                    pos.trailing_stop = pos.current_price * (1 + pos.trailing_step_bp / 10000)
            ...
            if pos.trailing_stop:
                if pos.side == "LONG":
                    ...
                    new_trail = pos.current_price * (1 - pos.trailing_step_bp / 10000)
            ...
            if pos.side == "LONG" and pos.current_price <= pos.stop_loss:
                ...
                # Target hit
            elif pos.side == "LONG" and pos.current_price >= pos.target:
                reason = "TARGET"
            elif pos.side == "SHORT" and pos.current_price <= pos.target:
                reason = "TARGET"

                # Trailing stop hit
            elif pos.trailing_stop:
                if pos.side == "LONG" and pos.current_price <= pos.trailing_stop:
                    reason = "TRAILING_STOP"
                elif pos.side == "SHORT" and pos.current_price >= pos.trailing_stop:
                    reason = "TRAILING_STOP"
    ...
    def close_position(self, index_symbol: str, pos_index: int, reason: str):
        ...
        stats.trades += 1
        stats.total_pnl_rupees += pnl_rupees

        if pnl_rupees > 0:
            stats.wins += 1
        else:
            stats.losses += 1
            if pnl_rupees < stats.max_drawdown_rupees:
                stats.max_drawdown_rupees = pnl_rupees
        ...
        print(f"[SCALP] CLOSE {pos.side} {pos.symbol} @ ₹{pos.current_price:.2f} | "
              f"PnL: ₹{pnl_rupees:+.2f} ({pnl_bp:+.1f}bp) | Reason: {reason} | "
...
class ScalpingOrchestrator:
    ...
    def __init__(self, fyers_client, demo_mode: bool = True):
    ...
    def update_tick(self, symbol: str, quote_data: dict):
        ...
        try:
            ...
            # Update option positions with current option price
            self.update_option_positions(symbol, ltp)

        except Exception as e:
            print(f"[TICK] Error processing {symbol}: {e}")

    def update_option_positions(self, index_symbol: str, index_price: float):
        """Fetch and update option prices for positions, then update position manager"""
        if index_symbol not in self.position_manager.positions:
            return

        positions = self.position_manager.positions[index_symbol]
        if not positions:
            return

            # Collect unique option symbols
        option_symbols = list(set([pos.symbol for pos in positions]))

        # Fetch quotes for all option positions
        try:
            if len(option_symbols) == 1:
                resp = self.fy.quotes(data={"symbols": option_symbols[0]})
            else:
                resp = self.fy.quotes(data={"symbols": ",".join(option_symbols)})

            if resp.get("s") == "ok" and resp.get("d"):
                quote_list = resp["d"] if isinstance(resp["d"], list) else [resp["d"]]

                # Build price map
                price_map = {}
                for quote in quote_list:
                    sym = quote.get('n')
                    ltp = float(quote.get('v', {}).get('lp', 0))
                    if sym and ltp > 0:
                        price_map[sym] = ltp

                        # Update all positions at once with the price map
                if price_map:
                    self.position_manager.update_positions(index_symbol, option_price_map=price_map)
        except Exception as e:
            # Silently continue if option price fetch fails
            pass
    ...
    def execute_trade(self, index_symbol: str, side: str, strategy: str, signal_data: list):
        ...
        try:
            ...
            try:
                opt_quote = self.fy.quotes(data={"symbols": opt_symbol})
                if opt_quote.get("s") == "ok" and opt_quote.get("d"):
                    quote_data = opt_quote["d"][0]["v"] if isinstance(opt_quote["d"], list) else opt_quote["d"]
                    option_price = float(quote_data.get('lp', 0))

                    if option_price <= 0:
                        print(f"[TRADE] Error: Invalid option price for {opt_symbol}")
                        return
                else:
                    print(f"[TRADE] Error: Could not fetch option price for {opt_symbol}")
                    return
            except Exception as e:
                print(f"[TRADE] Error fetching option quote: {e}")
                return

                # Place order (simulated here, implement actual order placement)
            order_side = "BUY"  # Always buy options for scalping

            print(f"[TRADE] Placing {order_side} {qty} {opt_symbol} @ Market (₹{option_price:.2f})")
            ...
            if not self.demo_mode:
                ...
                print(f"[DEMO] Simulated Order Placement for {opt_symbol}")

            # Open position with actual option premium
            metadata = {
            ...
            self.position_manager.open_position(
                ...
                index_symbol=index_symbol,
                side=side,
                entry_price=option_price,  # Actual option premium
                quantity=qty,
                strategy=strategy,
                index_entry_price=spot_price,  # Index price for reference
                metadata=metadata
            )
        ...
        try:
            ...
            if resp.get("data"):
                if isinstance(resp["data"], dict):
                    # Try both 'optionsChain' and 'optionChain' for compatibility
                    chain = resp["data"].get("optionsChain", resp["data"].get("optionChain", []))
                elif isinstance(resp["data"], list):
                    chain = resp["data"]

            if not chain:
                print(f"[ERROR] Option Chain Empty for {symbol_root}. Response structure: {list(resp.keys())}")
                if resp.get("data"):
                    print(
                        f"[DEBUG] Data type: {type(resp['data'])}, Keys: {list(resp['data'].keys()) if isinstance(resp['data'], dict) else 'list'}")
                return None, None
...
def main():
    ...
    parser.add_argument("--live", action="store_true", help="Enable LIVE trading (real money). Default is DEMO.")
    ...
    orchestrator = ScalpingOrchestrator(fy, demo_mode=not args.live)
    ...
    try:
            ...
            if cycle % 20 == 0:
                ...
                for symbol in SYMBOLS:
                    ...
                    if tick_mgr.last_tick:
                        ...
                        if stats:
                            status += f" | Trades: {stats.trades} | Win%: {stats.win_rate:.1f} | PnL: ₹{stats.total_pnl_rupees:+.2f}"
        ...
        for symbol, stats in orchestrator.position_manager.daily_stats.items():
            ...
            print(f"  Total PnL: ₹{stats.total_pnl_rupees:+.2f}")
            print(f"  Max DD: ₹{stats.max_drawdown_rupees:.2f}")
