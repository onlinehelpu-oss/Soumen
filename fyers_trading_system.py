...
try:
    ...
    print("WARNING: 'fyers_apiv3' library not found. Please install it using: pip install fyers_apiv3")
    data_ws = None
...
class FyersWebSocket:
    """Fyers WebSocket client using fyers_apiv3 library"""
    ...
    def _on_message(self, msg):
        """Handle incoming WebSocket messages from the library"""
        try:
            log_message(f"WebSocket message: {msg}", "DEBUG")

            # The library already provides formatted data
            ...
            elif isinstance(msg, dict) and msg.get('symbol'):
                formatted_quote = {
    ...
    def _on_connect(self):
        ...
        log_message("WebSocket connected. Subscribing to symbols...", "INFO")
        self.connected = True
        if self.symbols:
            try:
                self.fyers_socket.subscribe(symbols=self.symbols, data_type="SymbolUpdate")
                log_message(f"Subscribed to symbols: {self.symbols}", "INFO")
            except Exception as e:
                log_message(f"Subscription failed: {e}", "ERROR")

    def _on_close(self, msg):
        """Handle WebSocket closure"""
        log_message(f"WebSocket closed: {msg}", "WARNING")
        self.connected = False

    def _on_error(self, msg):
        """Handle WebSocket errors"""
        log_message(f"WebSocket error: {msg}", "ERROR")
        self.connected = False
    ...
    def connect(self):
        ...
        if data_ws is None:
            log_message("fyers_apiv3 library not installed. Cannot connect to WebSocket.", "ERROR")
            return False

        try:
            # The access token for the socket needs to be in the format APP_ID:ACCESS_TOKEN
            socket_access_token = f"{self.app_id}:{self.access_token}"

            self.fyers_socket = data_ws.FyersDataSocket(
                access_token=socket_access_token,
                log_path=os.path.join(LOG_DIR),
                litemode=True,
                write_to_file=True,  # Logs ws data
                reconnect=True,
            ...
            log_message("WebSocket connection timeout.", "ERROR")
            return False
    ...
    def add_symbol(self, symbol):
        ...
        if symbol not in self.symbols:
            ...
            if self.connected and self.fyers_socket:
                self.fyers_socket.subscribe(symbols=[symbol], data_type="SymbolUpdate")
    ...
    def remove_symbol(self, symbol):
        ...
        if symbol in self.symbols:
            ...
            if self.connected and self.fyers_socket:
                self.fyers_socket.unsubscribe(symbols=[symbol])
...
class TradingBot:
    ...
    def fetch_historical_data(self, days=365, resolution="D"):
        ...
        if data is None:
            log_message(f"Failed to fetch data for {self.symbol} from Fyers", "ERROR")
            return None
...
def main():
    ...
    print(
        "\n📦 Required packages: pip install fyers_apiv3 pandas numpy xgboost scikit-learn matplotlib joblib requests")
    print("=" * 70)
