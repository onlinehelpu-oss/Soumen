...
class SymbolMaster:
    ...
    def _download_master(self):
        ...
        fno_details = next(
            (item for item in resp.get("data", {}).get("symbolMaster", []) if item["feed_name"] == "FNO"), None)
...
def resolve_option_symbols(fyers: fyersModel.FyersModel, index_symbol: str, atm_strike: int) -> Tuple[str, str]:
    ...
    chain = chain_data.get("optionsChain", []) or chain_data.get("optionChain", [])
    if not chain:
    ...
    if future_options:
        ...
        if all_expiries:
            ...
            print(
                f"[{index_symbol}] WARNING: No future expiry found. Falling back to latest available expiry: {target_expiry.date()}")
        else:
    ...
    if not ce_options:
        raise RuntimeError(f"Could not find any CE options for the selected expiry of {index_symbol}.")
    if not pe_options:
        raise RuntimeError(f"Could not find any PE options for the selected expiry of {index_symbol}.")
...
if __name__ == "__main__":
    ...
    while True:
        ...
        for index_symbol, trade in list(STATE.active_trades.items()):
            ...
            print(
                f"Current Premium: {current_premium:.2f} (CE: {trade['ce_symbol']} @ {trade['ce_ltp']}, PE: {trade['pe_symbol']} @ {trade['pe_ltp']})")
            print(
                f"Gain: {(trade['sell_premium'] - current_premium):.2f}, MTM: {((trade['sell_premium'] - current_premium) * trade['qty']):.2f}")
