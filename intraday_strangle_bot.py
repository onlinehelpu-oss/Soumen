...
MAX_LOSS = 2000.0
ENABLE_TARGET_PROFIT = False  # Set to True to enable, False to disable
...
def extract_side_fields(item):
    ...
    return {"symbol": sym, "ltp": ltp, "oi": oi, "oich": oich, "oichp": oichp, "prev_oi": prev_oi,
...
class VolatilityEngine:
    ...
    def _get_past(self, symbol, field, seconds_ago):
        ...
        if abs(closest[
                   'ts'] - target_ts) > 300:  # If closest is > 5 mins away from target, treat as None or best effort
...
def print_and_save_chain_for_symbol(fy, symbol, S, num_strikes=8):
    ...
    pos = POSITIONS.setdefault(symbol, StranglePosition())
    ...
    if ce_atm.get("ltp") is not None and pe_atm.get("ltp") is not None:
        ...
        print(
            f"Should ATM options be BOUGHT today? {'YES' if is_buy else 'NO'} — ({prob_str} Probability, Expectancy {res['metrics']['exp_move']:.1f} vs Cost {res['metrics']['req_move']:.1f}).")
        ...
        if not pos.active and res["decision"] == "NO TRADE" and not blast and time_ok:
            ...
            if iv_now and iv_now > 0.18:
                ...
                for row in rows:
                    k = row["strike"]
                    d_ce = row.get("ce_delta")
                    if d_ce is not None:
                        diff = abs(d_ce - target_delta_ce)
                        if diff < min_diff_ce and diff < delta_tol:
                            min_diff_ce = diff
                            best_ce_k = k
                    d_pe = row.get("pe_delta")
                    if d_pe is not None:
                        diff = abs(d_pe - target_delta_pe)
                        if diff < min_diff_pe and diff < delta_tol:
                            min_diff_pe = diff
                            best_pe_k = k

                if best_ce_k is None: best_ce_k = atm_strike + (2 * step)
                if best_pe_k is None: best_pe_k = atm_strike - (2 * step)

                ce_sym = None
                pe_sym = None

                if best_ce_k in strikes_map and strikes_map[best_ce_k].get("CE"):
                     ce_sym = strikes_map[best_ce_k]["CE"].get("symbol")
                if best_pe_k in strikes_map and strikes_map[best_pe_k].get("PE"):
                ...
                if ce_sym and pe_sym:
                    ...
                    try:
                        ret1 = fy.place_order(data={
                            "symbol": ce_sym,
                            "qty": qty, "type": 2, "side": -1, "productType": "INTRADAY",
                            "limitPrice": 0, "stopPrice": 0, "validity": "DAY", "disclosedQty": 0, "offlineOrder": False
                        })
                        print(f"    CE Order: {ret1}")

                        ret2 = fy.place_order(data={
                            "symbol": pe_sym,
                            "qty": qty, "type": 2, "side": -1, "productType": "INTRADAY",
                            "limitPrice": 0, "stopPrice": 0, "validity": "DAY", "disclosedQty": 0, "offlineOrder": False
                        })
                        print(f"    PE Order: {ret2}")
        ...
        if pos.active:
            spot = S
            time_since_adj = time.time() - pos.last_adjust_time
            ...
            if time_since_adj > 120:
                ...
                # 🔵 PUT SIDE UNDER ATTACK (Roll Call Closer)
                    ...
                    if new_call_sym and is_option_contract(new_call_sym):
                         ...
                         print(f"    Cannot roll Call: New strike {new_call_strike} not found in chain.")
...
# ===============================
# POSITION ENGINE (Options Only Strangle)
# ===============================
