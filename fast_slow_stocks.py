...
def get_access_token(force_refresh=False) -> dict:
    tok = None
    ...
    if tok:
        # Best effort to find app_id
        app_id = get_app_id_from_config()
        return {"access_token": tok, "client_id": app_id}

    _real_print("[auth] No existing access token found (or forced refresh). Starting interactive login...")
    access_token, app_id = run_interactive_login()
    if not access_token:
        raise Exception(
            "Could not obtain access token even after interactive login."
        )
    return {"access_token": access_token, "client_id": app_id}
...
def _recreate_fyers_and_ws():
    ...
    try:
        ...
        # Fallback if client_id missing
        if not client_id:
             if ":" in (ACCESS_TOKEN or ""):
                 client_id = ACCESS_TOKEN.split(":")[0]
             else:
                 client_id = get_app_id_from_config() or "unknown_client_id"
    ...
    try:
        # Construct proper token for socket: AppID:AccessToken
    ...
    if args.test_table or fyersModel is None:
        ...
        if not client_id:
             if ":" in (ACCESS_TOKEN or ""):
                 ...
                 client_id = get_app_id_from_config() or ACCESS_TOKEN # Fallback
