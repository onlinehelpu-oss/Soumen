...
# ===================== CANDLE DETECTOR =====================
def is_bullish_hammer_candle(o, h, l, c, prev_o, prev_c, prev_h, prev_l, min_range_pct=0.0015):
    """
    GREEN hammer/pin-bar with:
      - Previous candle RED
      - Range filter to avoid tiny PREVIOUS bars
      - Close > Open (green)
      - (h - c) < (c - o)  and  (o - l) > (c - o)
    """
    if c == 0 or prev_c == 0:
    ...
    # Previous candle must be RED and not a tiny candle
    if prev_c >= prev_o:
        return False
    prev_rng = prev_h - prev_l
    if prev_rng <= 0 or (prev_rng / max(prev_c, 1e-9)) < min_range_pct:
        return False
                ...
                if prev_bar and prev_bar.get("c") is not None and prev_bar.get("o") is not None:
                    if prev_bar["c"] < prev_bar["o"]:
                        ...
                        if is_bullish_hammer_candle(
                            ...
                            prev_bar["o"], prev_bar["c"], prev_bar["h"], prev_bar["l"],
                            ...
                            try:
                                ...
                                if not hist.empty:
                                    ...
                                    if len(valid_idx) > 0:
                                        ...
                                        # Condition A: Open below VWAP and close above (classic crossover)
                                        cond_cross_and_close = (bar["o"] < vwap) and (bar["c"] > vwap)
                                        # Condition B: Open above VWAP, touch/test VWAP, and close above (support reclaim)
                                        cond_touch_and_reclaim = (bar["o"] > vwap) and (bar["l"] <= vwap) and (bar["c"] > vwap)

                                        if not (cond_cross_and_close or cond_touch_and_reclaim):
                                            print(f"[{dt.datetime.now():%H:%M:%S}] ⚠️ SKIP {sym} — signal did not meet VWAP criteria (vwap={vwap:.2f})")
                                        else:
