         ...
         if(imp_score >= InpImpulseScoreThreshold && imp_dir != SIGNAL_NONE)
           {
            ...
            m_setup.impulse_peak_price = tick.mid;

            double disp_pts = (m_symbol.Point() > 0) ? MathAbs(tick.mid - oldest_imp.mid) / m_symbol.Point() : 0.0;
            m_setup.impulse_displacement = MathMax(disp_pts, InpMinImpulseDisplacePts);

            m_setup.pullback_extreme_price = tick.mid;
            m_setup.pullback_micro_high_price = tick.mid;
            m_setup.pullback_micro_low_price = tick.mid;
            m_setup.pullback_start_time = TimeCurrent();

            if(InpEnableJournalLogs)
               PrintFormat("IMPULSE DETECTED: Score=%.1f, Dir=%s, Start=%.5f, Peak=%.5f, Disp=%.1f pts",
                           imp_score, (imp_dir == SIGNAL_BUY) ? "BUY" : "SELL", oldest_imp.mid, tick.mid, m_setup.impulse_displacement);
           }
         break;
        }
