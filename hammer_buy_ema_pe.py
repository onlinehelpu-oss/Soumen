...
*** LONG PE VERSION (BULLISH SIGNAL ON PE CHART) ***
"""
import os
import sys
import json
...
def get_strike_from_index_ltp(index_ltp: float, strike_distance: int = 0) -> float:
    ...
    # Adjust based on strike distance for PE
...
def run_tests():
    ...
    assert is_bullish_hammer_candle(140.0, 200.0, 100.0, 165.0, 200.0, 190.0) is False, "Test 4 Failed (Short Lower Wick)"

    # Test 5: Upper Wick too long (30%)
    # Range 100. L=100. O=150. C=170. H=200.
    # Upper Wick = 30/100 = 30% > 25%.
    # Lower Wick = 50%. Body = 20%.
    assert is_bullish_hammer_candle(150.0, 200.0, 100.0, 170.0, 200.0, 190.0) is False, "Test 5 Failed (Long Upper Wick)"
