# -*- coding: utf-8 -*-
"""
FAST-SLOW EMA STRATEGY - OPTIONS EXECUTION (FULLY RECTIFIED VERSION)
- Tracks NIFTY50, BANKNIFTY, FINNIFTY spot indices
- Executes trades in corresponding options
- Proper option price tracking and stop loss management
- STRICT NEXT CANDLE ENTRY
- Uses option price for stop loss and target (NOT spot price)
"""
...
def evaluate_on_new_candle(st: SymbolState):
    ...
    if st.status == "watch" and is_market_hours():
        ...
        if ema_sequence_ok and body_opens_below and body_closes_above and green_ok and range_ok:
            ...
            print(f"\n[SIGNAL] {st.symbol}: BULLISH ENTRY SIGNAL")
            print(f"  Spot: {curr_close:.2f}")
            print(f"  EMAs: Fast={ema_fast:.2f}, Slow={ema_slow:.2f}")
...
def main():
    ...
    # Parse arguments
    parser = argparse.ArgumentParser(description="Fast-Slow EMA Strategy - Options Execution (FULLY RECTIFIED)")
    ...
    print("\n" + "=" * 80)
    print("FAST-SLOW EMA STRATEGY - OPTIONS EXECUTION (FULLY RECTIFIED VERSION)")
    print("=" * 80)
    ...
    print(f"Fyers Lot Sizes: NIFTY=65, BANKNIFTY=30, FINNIFTY=60, SENSEX=20")
