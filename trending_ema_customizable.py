...
TIMEFRAME_MIN = 15  # Any TF in minutes (1,2,3,5,10,15,30,60,...)
...
SYMBOLS = SPOT_INDICES  # For compatibility with the rest of the script

# Correct Fyers lot sizes
MIN_LOT_SIZES = {
    "NSE:NIFTY50-INDEX": 25,
    "NSE:NIFTYBANK-INDEX": 15,
    "NSE:FINNIFTY-INDEX": 25,
}
...
def get_lot_size(index_symbol: str) -> int:
    ...
    return MIN_LOT_SIZES.get(index_symbol, 25)
...
def resolve_option_symbol(fyers: fyersModel.FyersModel, index_symbol: str, is_ce: bool, spot_ltp: float) -> Optional[str]:
    """
    Find the nearest CE/PE option for given index.
    Returns: option_symbol or None
    """
    root_map = {
...
class SymbolState:
    ...
    def __repr__(self):
        return f"<State {self.symbol} {self.status} qty={self.qty} sl={self.stop_price}>"
...
def decide_qty(symbol: str, entry_price: float) -> int:
    """Calculates the trade quantity in lots for index options."""
    if "INDEX" in symbol.upper():
        lot_size = get_lot_size(symbol)
        return lot_size * FIXED_QTY

    # Fallback for non-index symbols (original logic)
    if QTY_MAP.get(symbol) is not None:
        return max(0, int(QTY_MAP.get(symbol, 0)))
    ...
    if entry_price <= 0:
        return 0
    return max(0, int(alloc // entry_price))
            ...
            if sig_start is None:
                ...
                if next_allowed_bucket is None:
                    ...
                    if current_bucket < next_allowed_bucket:
                        ...
                        if ltp > trigger:
                            option_symbol = resolve_option_symbol(FYERS, symbol, is_ce=True, spot_ltp=ltp)
                            if not option_symbol:
                                _real_print(f"[entry-debug] {symbol} could not resolve option symbol; cancelling signal.")
                                state.status = "watch"
                                state.signal_candle = None
                                return
                            ...
                            _real_print(
                                ...
                                f"LTP {ltp} > signal_high {trigger} -> ENTRY on {option_symbol}"
                            )
                            ...
                            if isinstance(resp, dict) and resp.get("s") == "ok":
                                ...
                                _real_print(
                                    ...
                                    f"Entered at spot {state.entry_price:.2f} | "
                                    f"Stoploss on spot at {state.stop_price:.2f}"
                                )
        ...
        try:
            ...
            if exit_next_bucket is not None and current_bucket > exit_next_bucket:
                ...
                if ltp < exit_low:
                    ...
                    if isinstance(resp, dict) and resp.get("s") == "ok":
                        # P&L is unknown without option LTP, logging spot price difference
                        pnl = (ltp - state.entry_price) * (state.qty / get_lot_size(symbol))
                        _real_print(f"[{symbol}] EXIT OK. Spot PnL approx={pnl:.2f}")
                        state.status = "cooldown"
                        state.exit_pending = False
                        state.exit_signal_candle = None
                    else:
    ...
    if (
        ...
        try:
            if ltp <= state.stop_price:
                ...
                sell_resp = place_market_order(state.option_symbol, state.qty, side=-1)
                log_trade_event(
                    state.option_symbol, "STOP_SELL", state.qty, ltp, sell_resp
                )
                if isinstance(sell_resp, dict) and sell_resp.get("s") == "ok":
                    pnl = (ltp - state.entry_price) * (state.qty / get_lot_size(symbol))
                    _real_print(f"[{symbol}] STOP-LOSS SELL OK. Spot PnL approx={pnl:.2f}")
                    state.status = "cooldown"
...
def main():
    global TIMEFRAME_MIN, EXIT_EMA, ENTRY_FAST_EMA, EMA_BUFFER, MIN_RANGE_PCT, REQUIRE_GREEN_SIGNAL
