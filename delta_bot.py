    ...
    def process_tick(self, symbol, ltp, ts_val):
        try:
            ...
            if candle_start > p["ts"]:
                completed = p.copy()
                self.partial[symbol] = {"ts": candle_start, "open": ltp, "high": ltp, "low": ltp, "close": ltp,
                                        "ticks": 1}
                return completed
...
class DeltaClient:
    ...
    def fetch_positions(self):
        ...
        if not API_KEY or not API_SECRET:
            log("sync",
                "Skipping exchange sync. Set 'api_key' and 'api_secret' in config.json to detect manual closes.")
            return
        ...
        try:
            ...
            if data.get("success"):
                ...
                for sym, st in SYMBOL_STATES.items():
                    if st.status == "position":
                        # Check entry time to avoid race condition
                        if time.time() - st.entry_time < 30: continue

                        # If bot thinks it has a position, but API says NO (or size 0) -> Manual Close
                        if sym not in api_pos_map:
                            log("sync",
                                f"⚠️ Position for {sym} missing on exchange (Manual Close Detected). Resetting to Watch.")
                            st.status = "watch"
                            st.qty = 0
                            save_state()
                        else:
        ...
        if ltp < trigger:
            ...
            if not PAPER_TRADE and GLOBAL_CLIENT:
                ...
                if order:
                    ...
                    st.entry_price = ltp
                    st.entry_time = time.time()  # Track entry time to avoid race condition
            else:
                # PAPER TRADING
...
def main():
    ...
    if active_positions and (not API_KEY or not API_SECRET):
        print("\n" + "!" * 80)
        print(
            f"⚠️  CRITICAL WARNING: {len(active_positions)} Active Position(s) Restored ({', '.join(active_positions)})")
        ...
        while True:
            ...
            for sym in SYMBOLS_TO_MONITOR:
                ...
                if not st.data.empty:
                    ...
                    print(
                        f"[heartbeat]   {trend} {sym:<12} | LTP: $ {display_ltp:,.2f} | 24h Change: {icon} {chg:>+7.2f}% | Vol: {vol:,} | Status: {st.status}")
