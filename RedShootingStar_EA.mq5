...
input group "Candle Geometry (Rejection Shape - Code-2)"
...
input group "Flexible Candle Geometry (Code-1)"
...
//==============================================================//
// Flexible Long Upper Wick Rejection Candle Detector (Code-1)
...
bool IsLongUpperWickRejection(int shift=1)
{
   ...
   if(UpperPct < MinUpperWickPct)
      return(false);

   if(BodyPct > MaxBodyPct)
      return(false);

   if(LowerPct > MaxLowerWickPct)
      return(false);
...
void CheckForSignal()
{
   ...
   // Code-1 Candle Geometry Detection
   bool validCode1 = IsLongUpperWickRejection(1);

   // Code-2 Candle Geometry Detection
   ...
   bool validCode2 = (upperWickPct >= InpUpperWickMin && upperWickPct <= InpUpperWickMax) &&
                     (bodyPct >= InpBodyMin && bodyPct <= InpBodyMax) &&
                     (lowerWickPct >= 0 && lowerWickPct <= InpLowerWickMax);

   if(!validCode1 && !validCode2) return;
   ...
   PrintFormat("Fine-tuned Signal Detected: %s at %s. High: %.2f, EMA: %.2f, Close: %.2f (Code1 Match: %s, Code2 Match: %s)",
               _Symbol, TimeToString(m_signalBarTime), h, (InpUseEMAFilter ? iMA_EMA_Value(1) : 0), c,
               validCode1 ? "Yes" : "No", validCode2 ? "Yes" : "No");
}
