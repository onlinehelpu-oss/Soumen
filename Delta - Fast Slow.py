    ...
    try:
        ...
        if os.path.exists(config_path):
            with open(config_path, "r") as f:
                cfg = json.load(f)
                print(f"[init] Loaded configuration from {config_path}")
                return cfg
        else:
            print("[init] Configuration file not found. Using default settings.")
            return {}
    except Exception as e:
        print(f"[init] Error loading config.json: {e}. Using defaults.")
        return {}
...
# FIXED: Use full symbol list from code, ignore config to avoid override
...
def on_tick(symbol, ltp, ts):
    ...
    if st.status == "entry_pending" and st.signal_candle:
        ...
        if st.signal_expiry and st.signal_expiry.tzinfo is None and ts_obj and ts_obj.tzinfo is not None:
    ...
    if st.status == "position":
        ...
        if exit_price > 0:
            ...
            # FIXED: Reset exit flags when closing position
            st.status = "watch"
            st.qty = 0
...
def main():
    ...
    def on_ws_message(ws, message):
        try:
            ...
            # FIXED: Handle subscription confirmation messages
            msg_type = data.get("type")

            if msg_type == "subscriptions":
                # Just log but don't process
                return
            ...
            if msg_type == "v2/ticker":
                ...
                sym = data.get("symbol")
                # FIXED: Check if symbol exists in our monitored list
                if not sym or sym not in SYMBOL_STATES:
                    return
