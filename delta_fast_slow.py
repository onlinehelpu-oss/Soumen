...
def sync_positions():
    ...
    # Only fetch for active positions to save API calls and avoid rate limits
    ...
    for sym in active_symbols:
        ...
        try:
            ...
            # If successful response, we assume 0 unless found otherwise
            # This covers the "empty list" case (closed position)
            broker_map[sym] = 0
            ...
            if "result" in resp:
                ...
                for p in results:
                    # Double check product_id if present
                    r_pid = int(p.get("product_id", 0))
                    if r_pid == 0 or r_pid == int(pid):
                        size = int(p.get("size", 0))
                        broker_map[sym] = size
        except Exception as e:
            print(f"[sync] Failed to fetch position for {sym}: {e}")

    try:
        # Compare with Bot State
        for sym, st in SYMBOL_STATES.items():
            if st.status == "position":
                # Safety: If we failed to fetch (not in map), skip checks to avoid false close
