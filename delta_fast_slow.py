    ...
    if st.status == "position":
        ...
        if st.target_price and ltp >= st.target_price:
            ...
            if isinstance(resp, dict) and "result" in resp:
                ...
                print(
                    f"❌ [exit] Critical Error: Invalid API Key for {symbol}. Resetting state to WATCH to prevent loop.")
                st.status = "watch"
                st.qty = 0
                st.sl_order_id = None
                st.tp_order_id = None
...
def main():
    ...
    if ENABLE_LIVE_TRADING and (not API_KEY or not API_SECRET):
        ...
        print(
            "   -> Or manually update the API_KEY and API_SECRET constants in the script (not recommended for sharing).")
    ...
    # Heartbeat loop
    last_heartbeat_ts = 0
    # Run heartbeat logic every TIMEFRAME_MIN minutes
    heartbeat_interval = TIMEFRAME_MIN * 60

    while True:
        # Sync positions frequently (every 15s) to catch external fills
        sync_positions()

        # Check if it's time for heartbeat (ticker update & logs)
        now_ts = time.time()
        ...
        # Short sleep for responsive position sync
        time.sleep(15)
