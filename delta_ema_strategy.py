...
def handle_api_error(resp, context_msg=""):
    ...
    if isinstance(resp, dict) and "error" in resp:
        ...
        if code == "ip_not_whitelisted_for_api_key":
            ...
            print(
                f"   👉 SOLUTION: Go to Delta Exchange -> API Settings -> Edit Key -> DISABLE 'IP Access Restriction'.")
...
def compute_indicators(df):
    ...
    min_ok = rng >= MIN_RANGE_PCT if MIN_RANGE_PCT > 0 else True
    max_ok = rng <= MAX_RANGE_PCT if MAX_RANGE_PCT > 0 else True
    df["ok_signal"] = min_ok & max_ok
    return df
        ...
        try:
            ...
            if not (isinstance(resp, dict) and "result" in resp):
                ...
                if error_data.get("code") == "ip_not_whitelisted_for_api_key":
                    ...
                    print(
                        f"   👉 Action Required: Go to Delta Exchange -> API Settings -> Edit Key -> Add {ip} or Disable IP Whitelist.")
...
def evaluate_on_new_candle(st):
    ...
    if st.status == "watch":
        # 1. EMA Fast > EMA Slow
        # 2. Rising Slow EMA
        # 3. Candle OPEN < min(EMA_fast, EMA_slow) (Strict Body Cross)
        # 4. Close > Max(EMAs)
        ...
        if cond1 and cond2 and cond3 and cond4 and cond_green and ok_signal:
            ...
            if target > curr["high"]:
                ...
                print(
                    f"[signal] 🔵 ENTRY SIGNAL {st.symbol} | High: {curr['high']} | Target: {target} | Wait for break > High (Expires: {st.signal_expiry})")
...
def parse_args():
    ...
    parser.add_argument("--max-range-pct", type=float, default=MAX_RANGE_PCT,
                        help="Maximum candle range %% (default: 0.0)")
...
def main():
    global TIMEFRAME_MIN, EXIT_EMA, ENTRY_FAST_EMA, ENTRY_SLOW_EMA, MIN_RANGE_PCT, MAX_RANGE_PCT, EMA_BUFFER, TRAIL_ATR_MULT, ALLOC_DEFAULT, ALLOC_PCT, MIN_CHANGE_PCT, SYMBOLS
