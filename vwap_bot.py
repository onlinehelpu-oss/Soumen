...
def get_access_token():
    ...
    if os.path.exists(TOKEN_PATH):
        ...
        # Handle dictionary-based token
        if isinstance(token_data, dict) and 'access_token' in token_data:
            access_token = token_data['access_token']
        # Fix for malformed string token loaded from file (e.g., "APP_ID:TOKEN")
        elif isinstance(token_data, str) and ":" in token_data:
            print("[auth] Malformed token detected from file, attempting to fix...")
