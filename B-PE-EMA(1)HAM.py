...
class RealTimeOptionManager:
    ...
    def _parse_expiry_date(self, expiry_code: str, index_name: str = None) -> Optional[dt.datetime]:
        ...
        try:
            ...
            # Ensure it is YYMDD by checking if last 2 chars are digits (e.g. 26122)
            # This prevents DDMMM (e.g. 26JAN) from being parsed here
...
def make_onmsg(fy, option_manager: RealTimeOptionManager, options_data: Dict, dry_run=False):
    def onmsg(msg):
        ...
        if (prev_for_cross is not None) and (prev_for_cross <= threshold) and (ltp > threshold):
            ...
            # Side 1 = BUY
            order_resp = place_order(fy, sym, side=1, qty=qty_shares, tag="OptGreenHammerPE", dry_run=dry_run)
            ...
            if order_resp.get('s') == 'ok' and order_resp.get('code') == 1101:
                ...
                trigger.pop(sym, None)
                print(
                    f"[{tick_time:%H:%M:%S}] ✅ LONG-PE {sym} @ {entry_price:.2f}, SL={sl_price:.2f}, TGT={tgt_price:.2f}, QTY={qty_lots} lots ({qty_shares} shares), Lot Size={lot_size}")
            else:
...
def main():
    ...
    print("\n" + "=" * 70)
    print("🎯 GREEN-HAMMER (PE BUY) STRATEGY - REAL-TIME")
    print("=" * 70)
