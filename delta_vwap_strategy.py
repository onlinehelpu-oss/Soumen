# Delta Exchange VWAP Strategy (Modified from Fast/Slow EMA)
# -*- coding: utf-8 -*-
"""
- ENTRY (Bullish, using VWAP & ENTRY_SLOW_EMA):
    * Signal candle:
        - Candle CLOSE > VWAP (Body closes above VWAP)
        - ENTRY_SLOW_EMA < VWAP (Trend filter)
...
- EXIT:
    ...
    * NO FIXED TARGET (Swing High target removed).
      Exits via Stop Loss, Trailing SL, or EXIT EMA.

- EMAs:
    * ENTRY_SLOW_EMA – for entry filter (must be < VWAP).
...
# STRATEGY PARAMETERS
TIMEFRAME_MIN = 15  # Default 5m as per log example
EXIT_EMA = 50
# ENTRY_FAST_EMA = 9  <-- REMOVED
ENTRY_SLOW_EMA = 15
...
ALLOC_DEFAULT = 10.0  # From log example
...
def compute_indicators(df):
    ...
    # df["ema_fast_entry"] = ema(df["close"], ENTRY_FAST_EMA) <-- REMOVED
...
def sync_positions():
    ...
    if not ENABLE_LIVE_TRADING:
        ...
        # print("[sync] Synchronizing with Delta Exchange positions...") # Removed spammy print
...
def compute_prev_swing_high_for_entry(state, lookback, reference_price):
    # NOTE: This function is deprecated as Target condition is removed
    # But kept to avoid breaking calls if any remain or for future use
    df = state.data
    if df is None or df.empty:
        return float("nan")
...
def compute_prev_swing_low_for_entry(state, lookback, reference_price):
    df = state.data
    if df is None or df.empty:
        return float("nan")
    ...
    for i in range(pivot_width, len(lows) - pivot_width):
        ...
        is_valley = True
        for j in range(1, pivot_width + 1):
            if lows[i - j] <= curr or lows[i + j] <= curr:
                is_valley = False
                break
        if is_valley:
            valleys.append(curr)

    valid = [v for v in valleys if v < reference_price]
    if valid:
        return valid[-1]
...
def evaluate_on_new_candle(st):
    ...
    # ema_fast = curr["ema_fast_entry"] # REMOVED
    ema_slow = curr["ema_slow_entry"]
    # ema_slow_prev = prev["ema_slow_entry"]
    ema_exit = curr["ema_exit"]
    vwap = curr.get("vwap", curr["close"]) # Fallback if missing
    ...
    if st.status == "watch":
        ...
        cond_close_above_vwap = curr["close"] > vwap
        cond_slow_below_vwap = ema_slow < vwap

        cond_green = (curr["close"] > curr["open"]) if REQUIRE_GREEN_SIGNAL else True
        ok_signal = bool(curr.get("ok_signal", True))

        if cond_close_above_vwap and cond_slow_below_vwap and cond_green and ok_signal:
            # TARGET REMOVED as per request
            ...
            st.potential_target_price = None # No fixed target
            st.qty = 0  # Calculated at entry

            print(
                f"[signal] 🔵 ENTRY SIGNAL {st.symbol} | High: {curr['high']} | Target: NONE (Trend) | Wait for break > High (Expires: {st.signal_expiry})")
...
def place_market_order_wrapper(symbol, qty, side, reduce_only=False):
    ...
    if not ENABLE_LIVE_TRADING:
        print(
            f"[sim] Simulated {side.upper()} Order for {qty} {symbol} placed successfully. Reduce Only: {reduce_only}")
...
def place_take_profit_market_order(symbol, qty, side, stop_price):
    info = PRODUCT_MAP.get(symbol)
    if not info: return {"success": False}

    if not ENABLE_LIVE_TRADING:
        print(f"[sim] Simulated Take Profit Market Order for {qty} {symbol} at {stop_price}")
        return {
            "success": True,
            "result": {
    ...
    try:
        # TP Market Order: order_type='market_order', stop_order_type='take_profit_order', stop_price=trigger
        resp = CLIENT.place_order(
            product_id=info["id"],
            size=qty,
            side=side,
...
def on_tick(symbol, ltp):
        ...
        if ltp > trigger:
            ...
            if isinstance(resp, dict) and "result" in resp:
                ...
                if target_price:
                    ...
                    print(f"[entry] No Target Price. Skipping TP Order.")
    ...
    if st.status == "position":
        ...
        if st.stop_price and ltp <= st.stop_price:
            ...
            if st.sl_order_id:
                print(
                    f"[order] Internal SL Hit. Cancelling Exchange SL {st.sl_order_id} to Execute Market Sell (Safety)")
                cancel_order_wrapper(st.sl_order_id, symbol)
                st.sl_order_id = None
            if st.tp_order_id:
                cancel_order_wrapper(st.tp_order_id, symbol)
            ...
            if ltp >= (st.entry_price + dist):
                ...
                if st.sl_order_id and st.sl_order_id != "bracket-auto":
                    ...
                    if sl_resp.get("success", True) and "result" in sl_resp:
                        ...
                        print(
                            f"✅ [trail] Breakeven Stop Loss Placed on Exchange | ID: {st.sl_order_id} | Trigger: {st.stop_price}")
                    else:
...
def parse_args():
    parser = argparse.ArgumentParser(description="Delta Exchange VWAP + EMA Strategy Bot")
    ...
    # parser.add_argument("--entry-fast-ema", type=int, default=ENTRY_FAST_EMA,  <-- REMOVED
    #                     help="Entry Fast EMA period (default: 20)")
    parser.add_argument("--entry-slow-ema", type=int, default=ENTRY_SLOW_EMA,
                        help="Entry Slow EMA period (default: 50)")
    parser.add_argument("--min-range-pct", type=float, default=MIN_RANGE_PCT,
                        help="Minimum candle range %% (default: 0.0)")
    parser.add_argument("--ema-buffer", type=float, default=EMA_BUFFER, help="EMA Buffer (default: 0.0)")
    parser.add_argument("--trail-atr-mult", type=float, default=TRAIL_ATR_MULT,
                        help="Trailing ATR Multiplier (default: 1.0)")
    parser.add_argument("--allocation", type=float, default=ALLOC_DEFAULT,
...
def main():
    ...
    # ENTRY_FAST_EMA = args.entry_fast_ema # REMOVED
    ...
    if "result" in products and isinstance(products["result"], list):
        for p in products["result"]:
            sym = p.get("symbol")
            # Filter for likely valid trading pairs (Perpetual Futures)
            # We look for contract_type containing 'futures' or check symbol ending
            c_type = p.get("contract_type", "").lower()
            is_active = p.get("state") == "live"
    ...
    print(f"   - Strategy: VWAP + SlowEMA({ENTRY_SLOW_EMA}) Trend Follow")
