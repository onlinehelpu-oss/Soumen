...
void CheckAndExecuteBreakout()
  {
   ...
   double breakout_threshold = g_setup.signal_low - (InpEntryBufferPoints * m_symbol.Point());
   ...
   if(bid <= breakout_threshold)
     {
      ...
      double sl_distance_pts = (g_setup.signal_high - bid) / m_symbol.Point() + InpSLBufferPoints;
      ...
      double sl_price = bid + (sl_distance_pts * m_symbol.Point());
...
double CalculateLotSize(double risk_pts)
  {
   ...
   double tick_value = SymbolInfoDouble(_Symbol, SYMBOL_VALUE_TICK);
