...
def resolve_option_symbol(fyers: fyersModel.FyersModel, underlying: str, ltp: float) -> Optional[Dict[str, any]]:
    ...
    _real_print(f"[debug-options] No ITM CE options found for expiry {nearest_expiry_date}.")
    ...
    _real_print(
        f"[debug-options] CRITICAL FALLBACK: No ITM strikes available. Selecting closest available strike (ATM/OTM): {closest.get('symbol')}")
    return closest
...
def on_tick(symbol: str, ltp: float, ts: Optional[dt] = None):
    ...
    if state.status == "entry_pending" and state.signal_candle:
        ...
        if CANDLE_MANAGER._floor_ts(tick_ts) == next_candle_start and ltp > state.signal_candle["high"]:
            ...
            opt_symbol = option_details.get("symbol")
            lot_size = option_details.get("lot_size")

            if not lot_size or lot_size <= 0:
                _real_print(f"[{symbol}] Lot size not in option chain, using fallback.")
...
def evaluate_on_new_candle(st: SymbolState):
    ...
    if st.status == "position":
        ...
        if is_red and intrabar_up and closed_below:
            ...
            st.exit_signal_expiry = curr.name + \
                                    pd.Timedelta(minutes=TIMEFRAME_MIN)
