...
class DeltaClient:
    ...
    def fetch_history(self, symbol, timeframe_minutes, num_candles):
        ...
        if not api_res_str:
            ...
            log("warmup",
                f"Timeframe {timeframe_minutes}m not natively supported. Fetching {api_res_str} and resampling.")
...
def on_tick(symbol, ltp, ts):
    ...
    if st.status == "position":
        ...
        # BB Exit Trigger (Touch Upper BB or Signal)
        elif not st.data.empty and st.data.iloc[-1].get("upper_bb") and ltp >= st.data.iloc[-1]["upper_bb"]:
            st.force_exit = True
            exit_price = ltp
            reason = "BB UPPER TOUCH EXIT"
...
def main():
    ...
    for sym in SYMBOLS_TO_MONITOR:
        ...
        if not st.data.empty:
            last = st.data.iloc[-1]
            print(
                f"{sym:<12} | LTP: $ {last['close']:,.2f} | 24h Change: {st.change_24h:+.2f}% | Vol: {int(last.get('volume', 0)):,}")
    print("=" * 70)
    ...
    def on_ws_message(ws, message):
        try:
            ...
            if data.get("type") == "v2/ticker":
                ...
                change_24h = data.get("ltp_change_24h")
                ...
                if sym and ltp:
                    if change_24h is not None:
                        ...
                        if st:
                            try:
                                st.change_24h = float(change_24h)
                            except ValueError:
                                pass
