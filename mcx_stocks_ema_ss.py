...
def is_bearish_shooting_star_candle(o, h, l, c, prev_o, prev_c, min_range_pct=0.0015, ignore_prev_green=False):
    """
    RED shooting-star / pin-bar with UPDATED geometry (more realistic):
    - Previous candle GREEN (unless ignore_prev_green is True)
    - Current candle RED
    ...
    if c >= o:  # Current candle must be red
        return False
    if not ignore_prev_green and prev_c <= prev_o:  # Previous candle must be green
        return False
    if c == 0 or h <= l:
        return False
    total_range = h - l
        ...
        if tick_time >= cstart + dt.timedelta(minutes=TIMEFRAME_MIN) - dt.timedelta(seconds=1):
            if key not in processed_candles:
                ...
                if ONE_POSITION_AT_A_TIME and has_open_positions():
                    ...
                    # --- Signal Check ---
                # Added Regime EMA check: Signal valid only if Price (Close) < Regime EMA
                is_below_ema = True
                if new_ema:
                    if bar["c"] >= new_ema:
                        is_below_ema = False
                ...
                # Determine if it is the first candle of the session
                is_mcx_sym = sym.startswith("MCX:")
                session_start = dt.time(9, 0) if is_mcx_sym else dt.time(9, 15)
                is_first_candle = (cstart.time() == session_start)

                # Prepare previous candle values
                prev_o, prev_c = 0.0, 0.0
                has_prev_data = False
...
def run_tests():
    ...
    # Test 5: First candle scenario (previous candle not green, but ignored)
    # Current candle is valid Bearish Shooting Star (Red)
    # Previous candle is RED (would fail normally)
    # ignore_prev_green = True
    assert is_bearish_shooting_star_candle(100.0, 112.0, 90.0, 95.5, 98.0, 95.0, ignore_prev_green=True) is True, "Test 5 Failed"

    # Test 6: First candle scenario but Current candle is GREEN (should still fail)
    assert is_bearish_shooting_star_candle(100.0, 112.0, 90.0, 105.0, 98.0, 95.0, ignore_prev_green=True) is False, "Test 6 Failed"
