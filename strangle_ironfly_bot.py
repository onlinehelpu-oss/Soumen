...
class GammaBot:
    ...
    def sync_positions(self):
        ...
        try:
            ...
            has_short_put = 'put' in self.positions
            has_long_call = 'long_call' in self.positions
            has_long_put = 'long_put' in self.positions

            if has_short_call and has_short_put:
                if has_long_call and has_long_put:
                    self.state = StrategyState.IRON_FLY
                else:
                    self.state = StrategyState.STRANGLE_OPEN
            elif has_short_call or has_short_put:
                self.state = StrategyState.STRANGLE_OPEN
            else:
                self.state = StrategyState.WAITING
