...
# ============================== ORDER HELPERS ===============================
def get_traded_price(fyers: fyersModel.FyersModel, order_id: str, max_retries: int = 5, delay: int = 1) -> Optional[float]:
    """Polls the order book to get the traded price for a given order ID."""
    for _ in range(max_retries):
        try:
            orders = fyers.get_orders()
            if orders.get("s") == "ok":
                for order in orders.get("orderBook", []):
                    if order.get("id") == order_id and order.get("status") == 2:  # 2 = traded/filled
                        return float(order.get("tradedPrice", 0.0))
            time.sleep(delay)
        except Exception as e:
            print(f"[orders] Error fetching traded price for {order_id}: {e}")
            time.sleep(delay)
    print(f"[warning] Could not fetch traded price for order {order_id} after {max_retries} retries.")
    return None
...
class State:
    ...
    def reset_trade(self):
        ...
        self.side = None; self.opt_symbol = None; self.qty = 0; self.opt_entry_price = 0.0
        self.spos = 0
        ...
        if (c["open"] > ema5 and c["high"] > ema5 and c["low"] > ema5 and c["close"] > ema5
            ...
            try:
                ...
                if resp.get("s") == "ok":
                    ...
                    # --- CRITICAL: SET STATE *BEFORE* FETCHING PRICE TO PREVENT UNMANAGED POSITIONS ---
                    STATE.spos = STATE.sflag = 1
                    STATE.opt_symbol = ce_symbol
                    STATE.qty = lots
                    STATE.side = "sell_ce"
                    STATE.entry = ltp  # spot price at entry

                    # Now, fetch the traded price. A failure here is non-fatal for position management.
                    traded_price = get_traded_price(FYERS, order_id)
                    if traded_price:
                        STATE.opt_entry_price = traded_price
                        price_log = f"Opt Price={STATE.opt_entry_price:.2f}"
                    else:
                        # PnL will be inaccurate, but position is still managed
                        STATE.opt_entry_price = 0.0
                        price_log = "Opt Price=FETCH_FAILED"
                    ...
                    print(f"[SELL][TF={TIMEFRAME_MIN}m][R:R=1:{R_SELL:.2f}] LIVE ENTRY OK | CE={ce_symbol} | Spot LTP={STATE.entry:.2f} | {price_log} | SL={STATE.stoploss:.2f} TGT={STATE.target:.2f} | Lot={lots}")
                    print(f"Entry(spot)={STATE.entry:.2f}  Target(spot)={STATE.target:.2f}  SL(spot)={STATE.stoploss:.2f}")
                else:
    ...
    if STATE.spos == 1 and STATE.side == "sell_ce" and STATE.opt_symbol:
        ...
        if STATE.stoploss > 0 and ltp > STATE.stoploss:
            try:
                ...
                if resp.get("s") == "ok":
                    order_id = resp.get("id")
                    exit_price = get_traded_price(FYERS, order_id)
                    if exit_price and STATE.opt_entry_price > 0:
                        pnl = (STATE.opt_entry_price - exit_price) * STATE.qty
                        STATE.pnl_cum += pnl
                        print(f"[SELL] STOP OUT | PnL={pnl:.2f} | Cum PnL: {STATE.pnl_cum:.2f} | resp={resp}")
                    else:
                        print(f"[SELL] STOP OUT order placed. PnL could not be calculated (price fetch failed).")
                    STATE.reset_trade()
                else:
                    print(f"[order] STOP OUT (BUY CE) failed: {resp}")
            except Exception as e:
                print(f"[SELL] stop out error: {e}")
        # --- TARGET ---
            ...
            try:
                ...
                if resp.get("s") == "ok":
                    ...
                    if exit_price and STATE.opt_entry_price > 0:
                        pnl = (STATE.opt_entry_price - exit_price) * STATE.qty
                        STATE.pnl_cum += pnl
                        print(f"[SELL] TARGET HIT | PnL={pnl:.2f} | Cum PnL: {STATE.pnl_cum:.2f} | resp={resp}")
                    else:
                        print(f"[SELL] TARGET HIT order placed. PnL could not be calculated (price fetch failed).")
                    STATE.reset_trade()
                else:
                    print(f"[order] TARGET HIT (BUY CE) failed: {resp}")
            except Exception as e:
                print(f"[SELL] target hit error: {e}")
