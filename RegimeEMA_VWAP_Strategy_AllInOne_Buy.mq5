//+------------------------------------------------------------------+
//|                    RegimeEMA_VWAP_Strategy_AllInOne_Buy.mq5       |
//|  Single-file EA: plots VWAP + Swing High/Low directly on the      |
//|  chart AND runs the buy-side strategy - no separate indicator     |
//|  files required.                                                  |
//|                                                                    |
//|  RULES:                                                            |
//|   Signal candle:                                                  |
//|     - candle Low crosses below / touches the Regime EMA           |
//|       AND candle closes back ABOVE the EMA                        |
//|     - the signal candle must be GREEN (close > open)              |
//|     - the candle immediately BEFORE it must be RED                |
//|     - the signal candle Close must be ABOVE the VWAP computed     |
//|       on a user-selectable timeframe                              |
//|   Entry:                                                           |
//|     - Buy as soon as the very NEXT candle breaks above the        |
//|       signal candle's High (checked tick by tick, not on close)    |
//|     - if that next candle closes without breaking the high, the    |
//|       signal is discarded                                          |
//|   Stop Loss  : the signal candle's Low                            |
//|   Take Profit: the most recent confirmed Swing High prior to the   |
//|                signal candle                                       |
//|   Trade mgmt : once price moves 1:1 in favour of the trade, the    |
//|                Stop Loss is trailed to break-even (entry price)    |
//+------------------------------------------------------------------+
...
input bool                 InpRequireSwingTarget = true;    // Skip trade if no valid swing high found

input group "=== Trade management ==="
input double               InpLots           = 0.10;
input ulong                 InpMagic          = 990034;
input bool                  InpDrawObjects    = true;       // Draw all chart visuals
input bool                  InpUseBreakEven   = true;       // Trail SL to break-even once price moves 1:1 in favour
...
//--- Break-even tracking (per open trade)
double   g_beEntryPrice            = 0.0;
double   g_beInitialRiskDistance   = 0.0;
...
//+------------------------------------------------------------------+
//| Called every tick: draws a live segment from the last confirmed   |
//| VWAP point to the current (still-forming-bar) VWAP value, so the  |
//| VWAP line updates on the chart in real time, not just on bar close|
//+------------------------------------------------------------------+
void UpdateLiveVWAPDraw()
  {
   if(!InpDrawObjects)
      return;

   double liveVal = GetLiveVWAPValue();
   datetime tNow   = TimeCurrent();

   string name = "VWAP_seg_live";
   ObjectDelete(0, name);
   ObjectCreate(0, name, OBJ_TREND, 0, g_vwapPrevTime, g_vwapPrevValue, tNow, liveVal);
   ObjectSetInteger(0, name, OBJPROP_COLOR, InpVWAPColor);
   ObjectSetInteger(0, name, OBJPROP_WIDTH, 2);
   ObjectSetInteger(0, name, OBJPROP_RAY_RIGHT, false);
   ObjectSetInteger(0, name, OBJPROP_RAY_LEFT,  false);
...
//+------------------------------------------------------------------+
double GetPreviousSwingHigh(datetime beforeTime)
  {
   int n = ArraySize(g_swingHighTimes);
   for(int i = n - 1; i >= 0; i--)
     {
      if(g_swingHighTimes[i] <= beforeTime)
         return g_swingHighValues[i];
     }
   return -1.0;
  }
...
void SyncPositionLines()
  {
   if(HasOpenPosition())
   ...
   else if(g_positionLinesDrawn)
     {
      ...
      //--- position is gone: clear break-even tracking for the next trade
      g_beEntryPrice          = 0.0;
      g_beInitialRiskDistance = 0.0;
      g_beApplied             = false;
     }
  }
...
bool StopsAreValid(double entryPrice, double slPrice, double tpPrice)
  {
   ...
   if(tpPrice > 0.0 && (tpPrice - entryPrice) < minDistance)
      return false;
...
//+------------------------------------------------------------------+
//| Called every tick while a position is open: once price has moved  |
//| in favour of the (long) trade by an amount equal to the original  |
//| SL distance (1:1), move the Stop Loss to break-even (entry price). |
//+------------------------------------------------------------------+
void CheckBreakEven()
  {
   if(!InpUseBreakEven)
   ...
   if(g_beEntryPrice <= 0.0 || g_beInitialRiskDistance <= 0.0)
      return;

   //--- current market price relative to a buy position
   double bid = SymbolInfoDouble(_Symbol, SYMBOL_BID);
   double favourableDistance = bid - g_beEntryPrice; // positive = price has risen, in favour of the long

   if(favourableDistance < g_beInitialRiskDistance)
      return; // not yet reached 1:1

   double breakEvenPrice = g_beEntryPrice;
   double currentTP       = PositionGetDouble(POSITION_TP);
   double currentSL       = PositionGetDouble(POSITION_SL);

   //--- nothing to do if the SL is already at/beyond break-even
   ...
   if(trade.PositionModify(_Symbol, breakEvenPrice, currentTP))
     {
      g_beApplied = true;
      Print("Break-even reached (1:1): SL moved to entry price ", DoubleToString(breakEvenPrice, _Digits));
     }
   else
     {
      Print("Break-even SL modify failed. Error: ", GetLastError());
     }
  }
...
int OnInit()
  {
   ...
   //--- attach the Regime EMA directly to the main chart window so it
   //    is plotted and updates live (native indicator overlay, no manual redraw needed)
   if(InpDrawObjects)
...
void OnDeinit(const int reason)
  {
   //--- detach the EMA overlay added in OnInit (main window = subwindow 0)
   //    before releasing its handle, so no orphaned indicator is left on the chart
   int total = ChartIndicatorsTotal(0, 0);
   for(int i = total - 1; i >= 0; i--)
     {
      string indName = ChartIndicatorName(0, 0, i);
      if(ChartIndicatorGet(0, 0, indName) == g_emaHandle)
...
void OnNewBar()
  {
   ...
   bool touchedOrCrossedBelowEma = (low1 < emaVal);
   bool closedAboveEma           = (close1 > emaVal);
   bool isGreenCandle            = (close1 > open1);
   bool prevIsRedCandle          = (close2 < open2);

   double vwapVal = GetLiveVWAPValue();
   bool isAboveVwap              = (close1 > vwapVal);

   if(touchedOrCrossedBelowEma && closedAboveEma && isGreenCandle && prevIsRedCandle && isAboveVwap)
     {
      ...
      DrawMarker("sig_" + TimeToString(time1), time1, low1 - 5 * _Point, 217, clrOrange, "SIGNAL");
     }
  }

//+------------------------------------------------------------------+
//| Checked on every tick: watches for the high-break entry trigger   |
//+------------------------------------------------------------------+
void CheckIntrabarBreak()
  {
   ...
   if(ask < g_signalHigh)
      return; // high not broken yet - keep waiting within this same candle

   //--- high IS broken and VWAP filter was already satisfied on the signal candle.
   //    signal gets exactly ONE order attempt so a rejection can't spam OrderSend.
   double tp = GetPreviousSwingHigh(g_signalBarTime);
   if(InpRequireSwingTarget && tp <= 0.0)
     {
      Print("Signal skipped: no confirmed swing high found for a Take Profit target.");
      g_entryTriggeredForSignal = true;
      g_waitingForBreak         = false;
      return;
     }

   double sl      = g_signalLow;
   double tpFinal = (tp > 0.0) ? tp : 0.0;

   //--- respect the broker's minimum stop distance so the order isn't rejected.
   //    Distance to SL grows as price keeps rising, so don't give up on the
   ...
   if(trade.Buy(InpLots, _Symbol, ask, sl, tpFinal, "RegimeEMA+VWAP long"))
     {
      DrawMarker("entry_" + TimeToString(curBarTime), curBarTime, ask, 233, clrLime, "BUY");
      DrawPositionLines(sl, tpFinal);

      //--- arm break-even tracking for this trade: risk distance = entry - SL
      g_beEntryPrice          = ask;
      g_beInitialRiskDistance = ask - sl;
      g_beApplied             = false;
     }
   else
     {
