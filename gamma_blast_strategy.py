...
def get_access_token():
    ...
    try:
        ...
        print(f"Could not extract code: {e}"); sys.exit(1)
    try:
...
def call_with_retries(func, payload, max_attempts=HTTP_MAX_ATTEMPTS):
    ...
    while True:
        ...
        s = resp.get("s"); code = resp.get("code")
        if s == "ok": return resp, None
        err = {"status":"error","error_code":code, "message":resp.get("message")}
        print(f"DEBUG: call returned error (s='{s}', code={code}, message={resp.get('message')}). Attempt {attempt}/{max_attempts}")
        if attempt >= max_attempts: return None, err
...
def get_ltps_batched(fy, symbols):
    ...
    for s in to_fetch:
        ...
        first_map[s] = vlist[0]; first_variants.append(vlist[0])
    ...
    def _extract_map(dlist):
        ...
        for item in dlist or []:
            ...
            name = item.get("n"); v = item.get("v") or {}
    ...
    for s in to_fetch:
        ...
        if ltp is not None:
            _QUOTE_CACHE[s] = (ltp, now); result[s] = (ltp, None)
        else:
            failures.append(s)
    ...
    if failures:
        ...
        for s in failures:
            ...
            if ltp2 is not None:
                _QUOTE_CACHE[s] = (ltp2, now); result[s] = (ltp2, None)
            else:
...
def bs_price(S, K, T, r, q, sigma, option_type):
    ...
    if sigma <= 0:
        return math.exp(-r*T)*max((S*math.exp(-q*T)-K) if option_type=="CE" else (K-S*math.exp(-q*T)), 0.0)
...
def bs_delta(S, K, T, r, q, sigma, option_type):
    if T<=0: return 1.0 if (option_type=="CE" and S>K) else (0.0 if option_type=="CE" else (-1.0 if S<K else 0.0))
...
def extract_side_fields(item):
    ...
    if oich is None and oi is not None and prev_oi is not None:
        try: oich = float(oi) - float(prev_oi)
        except Exception: oich = None
...
def choose_best_expiry(response, same_day_cutoff=SAME_DAY_CUTOFF):
    ...
    def date_only(dt_str):
        try: return datetime.datetime.strptime(dt_str, "%d-%m-%Y").date()
...
def parse_optionchain_response_with_expiry(response):
    ...
    for item in options_chain:
        ...
        if isinstance(opt_type, str) and opt_type.strip():
            if opt_type.upper() == "CE": strikes[strike]["CE"] = side; placed=True
            elif opt_type.upper() == "PE": strikes[strike]["PE"] = side; placed=True
        if not placed:
            sym = item.get("symbol","")
            if isinstance(sym,str) and sym.endswith("CE"): strikes[strike]["CE"] = side
            elif isinstance(sym,str) and sym.endswith("PE"): strikes[strike]["PE"] = side
            else:
...
def _variants_for_optionchain(symbol):
    v = []
    if ":" in symbol:
        v.append(symbol); v.append(symbol.split(":",1)[-1])
    else:
        v.append(symbol); v.append("NSE:" + symbol)
    out, seen = [], set()
    for x in v:
        if x not in seen:
...
def aggregate_doi_around_atm(strikes_map, atm_strike, step, window=GB_STRIKES_AROUND_ATM):
    ...
    for K in sorted([k for k in strikes_map.keys() if lo <= k <= hi]):
        ...
        if ce_oich is not None:
            CE_sum += float(ce_oich); CE_abs += abs(float(ce_oich))
        if pe_oich is not None:
            PE_sum += float(pe_oich); PE_abs += abs(float(pe_oich))
...
def print_and_save_chain_for_symbol(fy, symbol, S, num_strikes=8):
        ...
        for item in ed:
            if isinstance(item, dict) and isinstance(item.get("date"), str):
                ...
                if T_try and T_try > MIN_T_YEARS:
                    expiry_date = item["date"]; T = T_try; break
    if T is None or T <= MIN_T_YEARS:
        T = 1.0/365.0; skip_iv = True
    ...
    for K in sorted([k for k in strikes_map.keys() if minK <= k <= maxK]):
        ...
        if ce_oich is None and ce.get("oi") is not None and ce.get("prev_oi") is not None:
            try: ce_oich = float(ce.get("oi")) - float(ce.get("prev_oi"))
            except Exception: ce_oich = None
        pe_oich = pe.get("oich")
        if pe_oich is None and pe.get("oi") is not None and pe.get("prev_oi") is not None:
            try: pe_oich = float(pe.get("oi")) - float(pe.get("prev_oi"))
            except Exception: pe_oich = None
