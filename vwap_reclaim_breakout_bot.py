...
STRATEGY (Multi-Index Option Price Based + Dynamic Strike Shifting)
  ...
  - Index-specific option premium ranges:
      * NIFTY: 180 to 250 premium range (target ~215)
      * SENSEX: 400 to 500 premium range (target ~450)
...
--------------------------------------------------------------------------
RECTIFICATION NOTES & WEBSOCKET RESILIENCE FIXES
  Issue Addressed:
    When Fyers WebSocket connection fails or drops (e.g. [WinError 10060]
    connection attempt timed out / party failed to respond), the SDK's
    built-in reconnect logic attempted 5 retries and failed, while the
    main-loop watchdog repeatedly spawned new WebSocket threads every 60s.
    Furthermore, the 90s stale feed watchdog permanently paused new trade
    entries even though REST API quote fetching (`get_ltp`) was working 100%.

  Fixes Applied:
    1. `ALLOW_REST_FALLBACK_ENTRIES`: New configuration setting (default True).
       When WebSocket is unavailable or times out, the bot operates seamlessly
       in REST Polling Mode (fetching fresh quotes via REST API every cycle)
       without pausing new trade entries or blocking risk management.
    2. Exponential Backoff & Thread Guard in `start_websocket()`:
       Prevents thread accumulation and log spamming when WebSocket fails to
       connect due to network/firewall timeouts ([WinError 10060]).
    3. `reconnect=False` in `FyersDataSocket`:
       Controlled reconnect logic with exponential backoff managed by the
       bot's watchdog rather than uncontrolled SDK internal retry loops.
    4. Token Format Sanitization:
       Ensures access token is formatted cleanly without duplicate client_id
       prefixes.
    5. Cache & REST Fallback in `get_ltp()`:
       Validates tick timestamp freshness (STALE_AFTER_SECONDS) and transparently
       falls back to REST quotes when ticks are stale or WebSocket is down.
--------------------------------------------------------------------------
"""
...
# --- Feed health & fallback settings ---
STALE_AFTER_SECONDS = 15          # a cached websocket tick older than this is not trusted; falls back to REST
WS_SILENCE_RESTART_SECONDS = 60   # if no websocket message at all for this long, attempt socket reconnect with backoff
FEED_STALE_PAUSE_SECONDS = 90     # fallback trigger threshold
ALLOW_REST_FALLBACK_ENTRIES = True # if True, allow NEW entries via REST polling if WebSocket is unavailable/blocked
...
class FyersBroker:
    ...
    REQUIRED_FIELDS = [
        ...
        ("redirect_uri", "Redirect URI (must match myapi.fyers.in app settings exactly): "),
    ]
    ...
    def __init__(self, config: dict):
        ...
        self.creds = None
        self.ws_socket = None
        # live_ltp stores (price, last_update_unix_ts) tuples
        self.live_ltp: Dict[str, Tuple[float, float]] = {}
        self.subscribed_symbols: List[str] = []
        # Timestamp of the last message received on the websocket (any symbol)
        self._ws_last_message_ts: float = 0.0
        self._ws_lock = threading.Lock()

        # Resiliency & state management for WebSocket connection attempts
        self._ws_is_connected: bool = False
        self._ws_connecting: bool = False
        self._last_ws_reconnect_attempt: float = 0.0
        self._ws_reconnect_cooldown: float = 15.0
    ...
    def login(self) -> str:
        ...
        if cached:
            logging.info("Reusing today's cached access token -- no browser, no PIN, no OTP needed.")
            self.access_token = cached
        else:
            logging.info("No cached token for today -- Fyers resets tokens daily, so one login is needed.")
            pasted = input(
                "\nIf you already generated today's access token yourself on myapi.fyers.in, paste it now and press Enter.\n"
    ...
    def start_websocket(self, symbols: Optional[List[str]] = None):
        """
        Thread-safe WebSocket launcher with exponential backoff and connection state tracking.
        Prevents thread accumulation and log spamming when WebSocket connections time out ([WinError 10060]).
        """
        ...
        with self._ws_lock:
            now = time.time()
            # If a connection attempt is currently in progress, do not spawn another
            if self._ws_connecting:
                logging.debug("WebSocket connection attempt already in progress. Skipping duplicate launch.")
                return

            # Respect exponential backoff cooldown between failed reconnection attempts
            if not self._ws_is_connected and (now - self._last_ws_reconnect_attempt) < self._ws_reconnect_cooldown:
                logging.debug(f"WebSocket reconnect cooling down ({int(self._ws_reconnect_cooldown - (now - self._last_ws_reconnect_attempt))}s remaining).")
                return
            ...
            if self.ws_socket is not None:
                try:
                    self.ws_socket.close_connection()
                except Exception:
                    pass
                self.ws_socket = None

            raw_token = self.access_token
            client_id = self.creds.get("client_id", "")
            if raw_token.startswith(f"{client_id}:"):
                token_str = raw_token
            else:
                token_str = f"{client_id}:{raw_token}"

            self._last_ws_reconnect_attempt = now
            ...
            def on_message(msg):
                if isinstance(msg, dict) and "symbol" in msg and "ltp" in msg:
                    try:
                        t_now = time.time()
                        self.live_ltp[msg["symbol"]] = (float(msg["ltp"]), t_now)
                        self._ws_last_message_ts = t_now
                        self._ws_is_connected = True
                    except Exception:
                        pass
            ...
            def on_open():
                ...
                self._ws_reconnect_cooldown = 15.0  # Reset cooldown on successful connection
                ...
                if self.ws_socket and self.subscribed_symbols:
                    try:
                        ...
                        logging.warning(f"WebSocket subscribe failed: {e}")

            def on_error(msg):
                self._ws_is_connected = False
                self._ws_connecting = False
                logging.warning(f"WebSocket feed message/error: {msg}")

            def on_close(msg):
                self._ws_is_connected = False
                self._ws_connecting = False
                self._ws_fail_count += 1
                # Exponential backoff up to 300s (5 minutes)
                self._ws_reconnect_cooldown = min(300.0, 15.0 * (2 ** min(self._ws_fail_count, 5)))
                logging.warning(
                    f"WebSocket disconnected/unavailable ({msg}). "
                    f"Next reconnect retry in {int(self._ws_reconnect_cooldown)}s. "
                    f"REST API polling will handle market data updates seamlessly."
                )
            ...
            try:
                self.ws_socket = data_ws.FyersDataSocket(
                    ...
                    litemode=True,
                    write_to_file=False,
                    reconnect=False,  # Managed cleanly with exponential backoff by main loop watchdog
                    on_connect=on_open,
                ...
                def run_socket():
                    try:
                        self.ws_socket.connect()
                    except Exception as ex:
                        self._ws_is_connected = False
                        self._ws_connecting = False
                        logging.warning(f"WebSocket connection thread exited: {ex}")

                ws_thread = threading.Thread(target=run_socket, daemon=True)
                ws_thread.start()
                logging.info(f"WebSocket connection initiated for symbols: {self.subscribed_symbols}")
            except Exception as e:
                self._ws_connecting = False
                self._ws_is_connected = False
                logging.warning(f"Failed to initiate WebSocket connection: {e}")

    def websocket_is_stale(self, max_silence_seconds: int) -> bool:
        """True if WebSocket is not connected or no message arrived within max_silence_seconds."""
        if not self._ws_is_connected or self._ws_last_message_ts <= 0:
            return True
        return (time.time() - self._ws_last_message_ts) > max_silence_seconds

    def subscribe_symbol(self, symbol: str):
        if symbol and symbol not in self.subscribed_symbols:
            self.subscribed_symbols.append(symbol)
            if self.ws_socket and self._ws_is_connected:
                try:
    ...
    def _official_login(self) -> str:
        ...
        if is_local:
            ...
            class Handler(BaseHTTPRequestHandler):
                def do_GET(self):
                    ...
                    self.end_headers()
                    self.wfile.write(b"<html><body>Login captured. You can close this tab and return to PyCharm.</body></html>")
        ...
        if "auth_code" not in captured:
            pasted = input(
                "Auto-capture didn't complete. Paste the full redirect URL (or just the code= value) from the browser here: "
            ).strip()
    ...
    def get_ltp(self, symbol: str) -> float:
        ...
        Trusts websocket-cached tick ONLY if fresher than STALE_AFTER_SECONDS.
        Otherwise transparently falls back to a REST quote call (`fyers.quotes`).
        """
        cached = self.live_ltp.get(symbol)
        if cached is not None:
            price, ts = cached
            if price > 0 and (time.time() - ts) <= STALE_AFTER_SECONDS:
                return price
        ...
        try:
            ...
            self.live_ltp[symbol] = (val, time.time())
            return val
...
def handle_signal_and_entry(broker: FyersBroker, store: PositionStore,
    ...
    if entries_paused:
        logging.warning(
            f"[FEED WATCHDOG] Breakout detected for {symbol} but new entries are paused "
            f"because live feed is stale and ALLOW_REST_FALLBACK_ENTRIES is False. "
            f"Skipping this entry for safety."
        )
        return
...
def main():
    ...
    # Notification state flags for watchdog logging
    _entries_paused_warned = False
    ...
    while True:
        try:
            ...
            # --- Feed health watchdog & REST fallback management -----------
            entries_paused = False
            if isinstance(broker, FyersBroker):
                if broker.websocket_is_stale(WS_SILENCE_RESTART_SECONDS):
                    broker.start_websocket()

                if broker.websocket_is_stale(FEED_STALE_PAUSE_SECONDS):
                    if ALLOW_REST_FALLBACK_ENTRIES:
                        entries_paused = False
                        if not _rest_fallback_warned:
                            logging.info(
                                f"[REST FALLBACK MODE] WebSocket feed is unavailable or stale. "
                                f"Bot is actively polling live prices via REST API (`fyers.quotes`). "
                                f"Trading operations (entries & exits) remain 100% active."
                            )
                            _rest_fallback_warned = True
                    else:
                        entries_paused = True
                        if not _entries_paused_warned:
                            logging.warning(
                                f"[FEED WATCHDOG] Live feed stale for >{FEED_STALE_PAUSE_SECONDS}s and "
                                f"ALLOW_REST_FALLBACK_ENTRIES is False. Pausing NEW entries until feed recovers."
                            )
                            _entries_paused_warned = True
                else:
                    if _entries_paused_warned or _rest_fallback_warned:
                        logging.info("[FEED WATCHDOG] Live WebSocket feed active -- operating on real-time tick streaming.")
                        _entries_paused_warned = False
                        _rest_fallback_warned = False
            # ---------------------------------------------------------------
            ...
            if store.has_open_position():
                ...
                # Exits always run seamlessly via REST fallback or WS
