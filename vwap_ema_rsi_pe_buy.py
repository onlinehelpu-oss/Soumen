# -*- coding: utf-8 -*-
"""
VWAP-EMA-RSI STRATEGY - PE BUY (BEARISH)
- Tracks NIFTY50, BANKNIFTY, FINNIFTY spot indices
- Executes trades in corresponding PE options
- Entry Signal: VWAP body cross (Down) with EMA and RSI confirmation
...
# RSI Parameters (BEARISH)
# For PE Buy, we typically want RSI to be dropping from neutral/high.
# CE used 40-55. Inverse would broadly be 45-60 or staying in 40-55 range as "bearish continuation".
# Let's default to 45-60 for Bearish Entry to capture the breakdown early.
RSI_PERIOD = 14
RSI_ENTRY_MIN = 40      # Adjusted for Bearish Entry Zone
RSI_ENTRY_MAX = 55      # Adjusted for Bearish Entry Zone
RSI_EXIT_MIN = 25       # Profit Target (Oversold)
RSI_EXIT_MAX = 35       # Profit Target (Oversold)
...
REQUIRE_RED_SIGNAL = True # We want a RED candle for PE Buy
...
LOG_FILE = "trade_log_pe.csv"
STATE_DUMP = "symbol_states_pe.json"
PARTIAL_CANDLES_FILE = "partial_candles_pe.json"
STATE_PERSISTENCE_FILE = "bot_state_pe.json"

# Strike selection: 0=ATM, 1=1st OTM, -1=1st ITM
STRIKE_DISTANCE = 0

# Position management
SL_MODE = "signal_high"  # "signal_high" or "swing_high" for PE
SWING_LOOKBACK = 5  # used for swing-high
    ...
    try:
        ...
        if STRIKE_DISTANCE != 0:
            try:
                ...
                # For PE: ITM is Higher Strike, OTM is Lower Strike.
                # If STRIKE_DISTANCE is positive (e.g. 1 OTM), for PE we want LOWER strike.
                # If STRIKE_DISTANCE is negative (e.g. -1 ITM), for PE we want HIGHER strike.
                # In list sorted by price: Lower index = Lower Price.
                # PE OTM (Lower Price) -> Lower Index.
                # PE ITM (Higher Price) -> Higher Index.
                # So if STRIKE_DISTANCE=1 (OTM), we want index - 1 ?
                # Let's standardize:
                # CE: +1 distance = 1 strike HIGHER (OTM). -1 distance = 1 strike LOWER (ITM).
                # PE: +1 distance = 1 strike LOWER (OTM). -1 distance = 1 strike HIGHER (ITM).

                # Logic used in CE script: target_index = atm_index + effective_distance
                # For CE, effective = distance.
                # For PE, effective = -distance.
...
def sync_with_broker_positions():
    ...
    try:
        ...
        open_positions_map = {pos['symbol']: pos for pos in broker_positions if pos['netQty'] != 0}

        for symbol, state in SYMBOL_STATES.items():
            if state.status == "position":
                if state.option_symbol not in open_positions_map:
                    ...
                    # Sync quantity and potential price mismatch for carry-forward support
                    pos = open_positions_map[state.option_symbol]
                    broker_qty = pos.get('netQty', 0)

                    # Update quantity if changed (partial manual close)
                    if broker_qty != state.qty:
                        print(f"[sync] Quantity mismatch for {state.option_symbol}. Local: {state.qty}, Broker: {broker_qty}. Syncing...")
                        state.qty = broker_qty

                    # Optional: Sync entry price if needed, but keeping original entry might be better for strategy tracking.
                    # However, for carry forward next day, getting broker's avg price is useful.
                    broker_avg = pos.get('avgPrice', 0.0) or pos.get('buyAvg', 0.0)
                    if broker_avg > 0 and state.option_entry_price == 0:
                        print(f"[sync] Syncing entry price from broker: {broker_avg}")
                        state.option_entry_price = broker_avg
...
def handle_spot_tick(symbol: str, ltp: float, ts: dt):
    ...
    if state.status == "entry_pending" and state.signal_candle is not None:
        try:
            ...
            if pd.to_datetime(candle_start) == next_allowed_start:
                # Check for breakout below signal low (BEARISH)
                signal_low = float(state.signal_candle["low"])
                if ltp < signal_low and is_market_hours():
                    print(f"\n[{symbol}] ENTRY TRIGGERED: LTP {ltp:.2f} < signal_low {signal_low:.2f}")

                    # Resolve option symbol for execution (PE)
                    try:
                        option_symbol, expiry, strike = resolve_option_symbol(FYERS, symbol, is_ce=False, spot_ltp=ltp)
                        ...
                        if isinstance(resp, dict) and resp.get("s") == "ok":
                            ...
                            if SL_MODE == "signal_high":
                                state.spot_stop_price = float(state.signal_candle["high"])
                            elif SL_MODE == "swing_high":
                                recent_highs = state.data['high'].tail(SWING_LOOKBACK)
                                state.spot_stop_price = recent_highs.max()
    ...
    if state.status == "position":
        # Trailing Stop Loss Logic (Short logic: if price drops, move SL down)
        # Entry Price - (ATR * Mult) = Target Price
        # If Price <= Target Price, SL = Entry Price.
        ...
        # Stop Loss Check (Price rising hits SL)
...
def evaluate_on_new_candle(st: SymbolState):
    ...
    # ENTRY SIGNAL (BEARISH - VWAP BODY CROSS DOWN)
    if st.status == "watch" and is_market_hours():
        open_above_vwap = curr_open >= vwap
        closed_below_vwap = curr_close < vwap
        fast_ema_above_vwap = ema_fast > vwap
        red_ok = (not REQUIRE_RED_SIGNAL) or (curr_close < curr_open)
        ok_signal = bool(curr.get("ok_signal", True))
        rsi_ok = RSI_ENTRY_MIN <= rsi_val <= RSI_ENTRY_MAX

        if open_above_vwap and closed_below_vwap and fast_ema_above_vwap and red_ok and ok_signal and rsi_ok:
            st.signal_candle = {
                "ts": curr.name,
                "open": curr_open,
                "high": curr_high,
            ...
            print(f"\n[SIGNAL] {st.symbol}: VWAP CROSS DOWN (PE) ENTRY SIGNAL")
            ...
            print(f"  Waiting for next candle breakdown...\n")

    # EXIT SIGNAL (RSI) - Based on spot price
    elif st.status == "position":
        # RSI Profit Taking for PE: RSI drops into Oversold territory
...
def main():
    ...
    print("\n" + "=" * 80)
    print("VWAP-EMA-RSI STRATEGY - PE BUY (BEARISH) EXECUTION")
    print("=" * 80)
