...
input bool            InpUseEMAFilter         = true;             // Use EMA Filter (ON/OFF)
input int             InpEMAPeriod            = 34;               // EMA Period (Exponential, Closing price)
input double          InpRiskRewardRatio      = 2.0;              // Risk to Reward Ratio (e.g. 2.0 for 1:2)
input double          InpEntryBufferPoints    = 0.0;              // Entry Buffer in Points (Points below Red Low)
input double          InpSLBufferPoints       = 0.0;              // Stop Loss Buffer in Points (Points above Red High)
...
int OnInit()
{
   ...
   m_ema_handle = iMA(_Symbol, InpTimeframe, InpEMAPeriod, 0, MODE_EMA, PRICE_CLOSE);
...
void OnTick()
{
   ...
   if(current_bar_time != m_last_checked_bar_time)
   {
      ...
      double low1   = GetLow(_Symbol, InpTimeframe, 1);
      double high1  = GetHigh(_Symbol, InpTimeframe, 1);

      double open2  = GetOpen(_Symbol, InpTimeframe, 2);
      double close2 = GetClose(_Symbol, InpTimeframe, 2);
      double low2   = GetLow(_Symbol, InpTimeframe, 2);
      ...
      if(open1 > 0 && close1 > 0 && open2 > 0 && close2 > 0)
      {
         ...
         // Strategy Check:
         // 1. Previous candle (index 2) is Green (close2 > open2)
         // 2. Current signal candle (index 1) is Red (close1 < open1)
         // 3. Red candle (index 1) breaks the low of Green candle (index 2) (low1 < low2)
         if(close2 > open2 && close1 < open1 && low1 < low2)
         {
            bool ema_ok = true;
            double ema1 = 0.0;
            if(InpUseEMAFilter)
            ...
            if(ema_ok)
            {
               ...
               if(swing_ok)
               {
                  ...
                  m_breakout_level = low1 - InpEntryBufferPoints * symbol_point;
                  m_sl_level = high1 + InpSLBufferPoints * symbol_point;

                  Print("New Valid Setup Spotted!");
                  Print("  Signal Time (Red Candle): ", m_setup_time);
                  Print("  Green Low (index 2): ", low2);
                  Print("  Red Low (index 1): ", low1);
                  Print("  Red High (SL): ", high1);
                  Print("  Breakout Level: ", m_breakout_level);
                  if(InpUseEMAFilter)
                  {
                     Print("  EMA (", InpEMAPeriod, ") Check: PASSED (Close: ", close1, " < EMA: ", ema1, ")");
                  }
                  if(InpUseSwingHighFilter)
                  {
...
//+------------------------------------------------------------------+
//| Retrieve EMA Value at Specific Index                             |
//+------------------------------------------------------------------+
double GetEMA(int index)
{
   if(m_ema_handle == INVALID_HANDLE) return 0.0;
   double val[];
   ArraySetAsSeries(val, true);
   int copied = CopyBuffer(m_ema_handle, 0, index, 1, val);
   if(copied > 0) return val[0];
   return 0.0;
}
...
void UpdateDashboard()
{
   ...
   text += " EMA Filter: " + (InpUseEMAFilter ? "ON (Period: " + IntegerToString(InpEMAPeriod) + ")" : "OFF") + "\n";
