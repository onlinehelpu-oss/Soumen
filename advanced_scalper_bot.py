...
TICK_VELOCITY_THRESHOLD = 5  # ticks/second for momentum
...
QUOTE_REFRESH_SEC = 1  # 1 second for quotes
...
def get_access_token():
    ...
    # Manual auth flow (simplified)
    print("\n[AUTH] Token not found. Please login via Fyers portal and get token manually.")
    print("Save token in:", TOKEN_PATH)
    sys.exit(1)
...
class MicrostructureAnalyzer:
    ...
    def analyze(self, tick_mgr: TickDataManager) -> Optional[MicrostructureEdge]:
        ...
        if tick_vel > TICK_VELOCITY_THRESHOLD and price_change > 5:
            signal = "MOMENTUM_LONG"
            confidence = min(tick_vel / (TICK_VELOCITY_THRESHOLD * 2), 1.0)

        elif tick_vel > TICK_VELOCITY_THRESHOLD and price_change < -5:
