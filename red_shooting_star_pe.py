...
def get_strike_from_index_ltp(index_ltp: float, strike_distance: int = 0) -> float:
    ...
    For PE selling:
    - Negative distance: ITM strikes (higher strike than index LTP)
    - Zero distance: ATM strike (closest to index LTP)
    - Positive distance: OTM strikes (lower strike than index LTP)
    ...
    def get_option_details(self, index_symbol: str, strike_distance: int = 0, option_type: str = "PE") -> Optional[
        ...
        try:
            ...
            if strike_distance != 0:
                ...
                if strike_distance < 0:
                    # ITM: For PE selling, we want higher strikes
                    target_strike = atm_strike + (abs(strike_distance) * strike_interval)
                    strike_type = "ITM"
                else:
                    # OTM: For PE selling, we want lower strikes
                    target_strike = atm_strike - (strike_distance * strike_interval)
                    strike_type = "OTM"
...
def monitor_loop(fy, option_manager: RealTimeOptionManager, options_data: Dict, strike_distance: int, dry_run=False):
    ...
    while True:
        try:
            ...
            if (not FORCE_CLOSED_ALL) and (now_time >= EXIT_ALL_TIME):
                ...
                if open_trades:
                    ...
                    for sym in open_trades:
                        ...
                        if not trade:
                            continue
                        qty_lots = trade.get("qty")  # This is NUMBER OF LOTS
                        ...
                        try:
                            ...
                            print(
                                f"[{now_dt:%H:%M:%S}] 🔔 Force exiting {sym} (COVER BUY market) qty={qty_lots} lots ({total_shares} shares)")
                            # Pass both qty_lots and lot_size to exit function
            ...
            # ONLY monitor trades that were successfully placed
            if active_trades:
...
def main():
    ...
    print("\n" + "=" * 70)
    print("🎯 RED-SHOOTING STAR PE-SELLING STRATEGY - REAL-TIME")
    print("=" * 70)
...
def run_tests():
    ...
    # Test 1: Valid shooting star with corrected geometry
    # Upper: 60%, Body: 20%, Lower: 20%
    assert is_bearish_shooting_star_candle(100.0, 112.0, 92.0, 96.0, 95.0, 98.0) is True, "Test 1 Failed"
