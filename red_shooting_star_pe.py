...
PE VERSION - TRACKING PUT OPTIONS INSTEAD OF CALL OPTIONS
"""
import os
import sys
import json
...
# ===================== OPTION SETTINGS =====================
# For PE selling, a negative distance means ITM (In-the-Money)
...
def get_strike_from_index_ltp(index_ltp: float, strike_distance: int = 0) -> float:
    ...
    For PE selling:
    - Negative distance: ITM strikes (lower than index LTP)
    - Zero distance: ATM strike (closest to index LTP)
    - Positive distance: OTM strikes (higher than index LTP)
    ...
    # Adjust based on strike distance (REVERSED FOR PE SELLING)
    if strike_distance < 0:
        # ITM: Lower strike for PE selling
        target_strike = atm_strike - (abs(strike_distance) * strike_interval)
    elif strike_distance > 0:
        # OTM: Higher strike for PE selling
...
class RealTimeOptionManager:
    ...
    def get_option_details(self, index_symbol: str, strike_distance: int = 0, option_type: str = "PE") -> Optional[
        ...
        try:
            ...
            # Calculate target strike based on distance (REVERSED FOR PE SELLING)
            if strike_distance != 0:
                # Calculate ATM strike first
                atm_strike = round(index_ltp / strike_interval) * strike_interval

                if strike_distance < 0:
                    # ITM: For PE selling, we want lower strikes
                    target_strike = atm_strike - (abs(strike_distance) * strike_interval)
                    strike_type = "ITM"
                else:
                    # OTM: For PE selling, we want higher strikes
                    target_strike = atm_strike + (strike_distance * strike_interval)
                    strike_type = "OTM"
            ...
            if not options:
                ...
                # Filter by option type (CHANGED TO PE)
            filtered_options = [
            ...
            # Calculate strike position relative to index LTP (REVERSED FOR PE SELLING)
            if strike < index_ltp:
    ...
    def _parse_option_symbol(self, symbol: str, strike: float) -> Optional[Dict]:
        ...
        try:
            ...
            if symbol.startswith('NSE:'):
                ...
                # Determine option type (CHANGED TO CHECK FOR PE INSTEAD OF CE)
            if clean_symbol.endswith('PE'):
                option_type = 'PE'
                base_symbol = clean_symbol[:-2]
            elif clean_symbol.endswith('CE'):
                option_type = 'CE'
                base_symbol = clean_symbol[:-2]
            else:
    ...
    def refresh_options(self, indices: List[str], strike_distance: int = 0) -> Dict[str, Dict]:
        ...
        for index in indices:
            print(f"\n🔍 Refreshing {index}...")
            # CHANGED: Get PE options instead of CE
...
def make_onmsg(fy, option_manager: RealTimeOptionManager, options_data: Dict, dry_run=False):
    def onmsg(msg):
        if msg.get("type") != "sf":
            return
        try:
            sym = msg["symbol"]  # This is now an OPTION symbol (PE)
            ltp = float(msg["ltp"])
        ...
        if (prev_for_cross is not None) and (prev_for_cross >= threshold) and (ltp < threshold):
            ...
            # 3. Place order and save trade for the OPTION (SELLING PE)
            # Use qty_shares (total shares) for Fyers API
            order_resp = place_order(fy, sym, side=-1, qty=qty_shares, tag="PE-RedShootSell", dry_run=dry_run)
...
def main():
    ...
    print("\n" + "=" * 70)
    print("🎯 RED-SHOOTING STAR OPTIONS STRATEGY - REAL-TIME (PE VERSION)")
    print("=" * 70)
    ...
    print("=" * 70)
    print("🚀 Real-time SHORT scanner started (PE SELLING) …\n")
