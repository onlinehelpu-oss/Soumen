...
UPDATED: Hammer geometry (50-80% lower wick, 5-30% body, 0-25% upper wick)
WITH FIXED MARGIN SHORTFALL HANDLING
*** LONG CE VERSION (BULLISH BIAS) ***
"""
import os
import sys
import json
...
# UPDATED: Hammer geometry (Bullish)
# Upper Wick: 0-25% (Small upper shadow)
# Body: 5-30% (Small to medium body)
# Lower Wick: 50-80% (Long lower shadow)

UPPER_WICK_MAX = 25  # was LOWER_WICK_MAX in shooting star (0-25%)
BODY_MIN = 5
BODY_MAX = 30
LOWER_WICK_MIN = 50  # was UPPER_WICK_MIN in shooting star (50-80%)
LOWER_WICK_MAX = 80  # was UPPER_WICK_MAX in shooting star
    ...
    def _parse_expiry_date(self, expiry_code: str, index_name: str = None) -> Optional[dt.datetime]:
        ...
        try:
            ...
            if index_name == 'SENSEX' and len(expiry_code) == 5:
                # Check if it really looks like YYMDD (last 2 are digits, middle is digit or O/N/D)
                # '26JAN' has len 5 but is DDMMM, so it should fail this check and fall through.
                m_char_candidate = expiry_code[2].upper()
                dd_candidate = expiry_code[3:]

                is_yymdd = False
                if dd_candidate.isdigit():
                    if m_char_candidate.isdigit() or m_char_candidate in ['O', 'N', 'D']:
                        is_yymdd = True

                if is_yymdd:
                    # YYMDD format
                    yy = int(expiry_code[:2])
                    m_char = m_char_candidate
                    dd = int(dd_candidate)
...
def is_bullish_hammer_candle(o, h, l, c, prev_o, prev_c, min_range_pct=0.0015):
    """
    GREEN Hammer / Pin-bar with UPDATED geometry (Bullish):
    - Previous candle RED (Close < Open)
    - Current candle GREEN (Close > Open)
    - Geometry Constraints (as % of total candle range H-L):
      - Upper Wick: 0% - 25% (Small)
      - Body: 5% - 30% (Small to Medium)
      - Lower Wick: 50% - 80% (Long)
    """
    # --- Initial Filters ---
    ...
    is_valid_geometry = (
            (0 <= upper_wick_pct <= UPPER_WICK_MAX) and
            (BODY_MIN <= body_pct <= BODY_MAX) and
            (LOWER_WICK_MIN <= lower_wick_pct <= LOWER_WICK_MAX)
    )
    ...
    def onmsg(msg):
        ...
        if tick_time >= cstart + dt.timedelta(minutes=TIMEFRAME_MIN) - dt.timedelta(seconds=1):
            if key not in processed_candles:
                ...
                # "if price ABOVE Regime Ema, then if signal generate then only signal valid"
                is_above_ema = current_close > new_ema
                ...
                if prev_bar and is_bullish_hammer_candle(
                    ...
                    print(
                        f"[{tick_time:%H:%M:%S}] 🎯 OPTION-SIG {sym} TF={TIMEFRAME_MIN}m → watch NEXT HIGH {bar['h']:.2f} (SL {bar['l']:.2f})")
                    # Log candle geometry for debugging
                    total_range = bar["h"] - bar["l"]
                    if total_range > 0:
                        # Using Green candle perspective
        ...
        if (prev_for_cross is not None) and (prev_for_cross <= threshold) and (ltp > threshold):
            ...
            # Side 1 = BUY
            order_resp = place_order(fy, sym, side=1, qty=qty_shares, tag="OptGreenHammerCE", dry_run=dry_run)
    ...
    while True:
        try:
            ...
            if now - last_refresh > refresh_interval:
                ...
                print(
                    f"📊 Candle Geometry: Upper={0}-{UPPER_WICK_MAX}%, Body={BODY_MIN}-{BODY_MAX}%, Lower={LOWER_WICK_MIN}-{LOWER_WICK_MAX}%")
...
def run_tests():
    ...
    # Test 1: Valid Hammer
    # Prev Red: 100 -> 95
    # Curr Green: 90 -> 98 (Range 8)
    # Open 90, Close 98, High 100, Low 84 (Range 16)
    # Upper Wick (H-C): 100-98=2 (12.5%) - OK (<25%)
    # Body (C-O): 98-90=8 (50%) - NO, Body Max is 30%.

    # Let's make a valid one.
    # Total Range 100.
    # Lower Wick 60% (60 pts).
    # Body 25% (25 pts).
    # Upper Wick 15% (15 pts).

    # Low = 100
    # Open = 160 (Lower wick 60)
    # Close = 185 (Body 25)
    # High = 200 (Upper wick 15)

    # Prev candle Red (200 -> 190)

    assert is_bullish_hammer_candle(160.0, 200.0, 100.0, 185.0, 200.0, 190.0) is True, "Test 1 Failed (Valid Hammer)"

    # Test 2: Previous candle Green (Invalid)
    assert is_bullish_hammer_candle(160.0, 200.0, 100.0, 185.0, 190.0, 200.0) is False, "Test 2 Failed (Prev Green)"

    # Test 3: Current candle Red (Invalid)
    assert is_bullish_hammer_candle(185.0, 200.0, 100.0, 160.0, 200.0, 190.0) is False, "Test 3 Failed (Curr Red)"

    # Test 4: Lower Wick too short (40%)
    # Range 100. L=100. O=140. C=165. H=200.
    # Lower Wick = 40/100 = 40% < 50%.
    assert is_bullish_hammer_candle(140.0, 200.0, 100.0, 165.0, 200.0,
                                    190.0) is False, "Test 4 Failed (Short Lower Wick)"

    # Test 5: Upper Wick too long (30%)
    # Range 100. L=100. O=150. C=170. H=200.
    # Upper Wick = 30/100 = 30% > 25%.
    # Lower Wick = 50%. Body = 20%.
    assert is_bullish_hammer_candle(150.0, 200.0, 100.0, 170.0, 200.0,
                                    190.0) is False, "Test 5 Failed (Long Upper Wick)"
